...
# weight 경로: project weights 폴더 우선, 없으면 torch hub cache
...
_EFFNET_B0_PROJECT_PATH = (
    Path(__file__).parent.parent / "weights" / _EFFNET_B0_CACHE_FILENAME
)

def _get_effnet_weight_path() -> Path:
    if _EFFNET_B0_PROJECT_PATH.exists():
        return _EFFNET_B0_PROJECT_PATH
    if _EFFNET_B0_CACHE_PATH.exists():
        return _EFFNET_B0_CACHE_PATH
    return None
...
class FeatureExtractorEffNetB0:
    ...
    def __init__(self, device: str = None) -> None:
        weight_path = _get_effnet_weight_path()
        if weight_path is None:
            raise RuntimeError(
                f"EfficientNet-B0 weight 파일이 없습니다.\n"
                f"다음 위치 중 하나에 파일을 복사하세요:\n"
                f"  (1) {_EFFNET_B0_PROJECT_PATH}\n"
                f"  (2) {_EFFNET_B0_CACHE_PATH}\n"
                f"파일명: {_EFFNET_B0_CACHE_FILENAME}"
            )
