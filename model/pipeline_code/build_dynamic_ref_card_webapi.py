...
def render(patient, score_csv, position_bin=None, local_z=None, out_root=None,
           hu_npy_path=None, roi_npy_path=None, spacing_xy=None, out_png_path=None):
    t0 = time.time()
    ...
    # CT/ROI 로드: webapi 모드는 hu_npy_path/roi_npy_path 직접 사용, 아니면 CTBASE 경로
    errors = []
    if hu_npy_path and roi_npy_path:
        ct  = np.load(hu_npy_path, mmap_mode="r")
        roi = np.load(roi_npy_path, mmap_mode="r")
        cand_sp = spacing_xy if spacing_xy else 0.977
    else:
    ...
    # ── 저장 ─────────────────────────────────────────────────────────────────
    if out_png_path:
        # webapi 모드: 직접 PNG 경로로 저장
        os.makedirs(os.path.dirname(out_png_path), exist_ok=True)
        out_png  = out_png_path
        out_root = out_root or os.path.dirname(out_png_path)
        json_dir = out_root
        out_json = os.path.join(out_root, f"{patient}__{pbin}{SUFFIX}.json")
    else:
...
def main():
    ...
    ap.add_argument("--out_png",      default=None, help="직접 PNG 출력 경로 (out_root 대신)")
    ap.add_argument("--hu_npy",       default=None, help="CT HU 볼륨 npy 경로 (CTBASE 대신)")
    ap.add_argument("--roi_npy",      default=None, help="ROI 마스크 npy 경로 (CTBASE 대신)")
    ap.add_argument("--spacing",      type=float, default=None, help="XY spacing mm (hu_npy 사용 시)")
    ap.add_argument("--webapi",       action="store_true", help="웹API 모드: ALLOW 체크 생략, 전 플래그 활성")
    ap.add_argument("--render",       action="store_true")
    ap.add_argument("--confirm",      action="store_true")
    a = ap.parse_args()
    if a.webapi:
        for k in ALLOW: ALLOW[k] = True
    if (a.render and a.confirm) or a.webapi:
        render(a.patient, a.score_csv, a.position_bin, a.local_z, a.out_root,
               hu_npy_path=a.hu_npy, roi_npy_path=a.roi_npy, spacing_xy=a.spacing, out_png_path=a.out_png)
        sys.exit(0)
