#!/usr/bin/env python3
"""
B1-E12: 3축 local MIP + single-slice correction vessel mask generation
...
3축 local MIP 구현 (broadcast 미사용, 모두 (Y,X) 반환):
  axial:    maximum_filter1d(ct_arr, size=10, axis=0)[local_z] → (Y, X)
  coronal:  maximum_filter1d(ct_arr, size=10, axis=1)[local_z] → (Y, X)
  sagittal: maximum_filter1d(ct_arr, size=10, axis=2)[local_z] → (Y, X)
  최종 mask: 반드시 refined_roi[local_z] 내부로 제한
...
_STEM = "b1e12_triplanar_local_mip_single_slice_correction"
...
def compute_triplanar_local_mips(ct_vol, roi_vol, local_z):
    """
    3축 local MIP: maximum_filter1d를 3D volume에 axis별 적용
      axial:    axis=0 (z방향) [local_z] → (Y, X)
      coronal:  axis=1 (y방향) [local_z] → (Y, X)
      sagittal: axis=2 (x방향) [local_z] → (Y, X)
    broadcast 미사용, 모든 결과 (Y, X) 유지
    """
    n_z = ct_vol.shape[0]
    ct_arr = np.array(ct_vol).astype(np.float32)  # (Z, Y, X) 전체 로드

    axial_mip    = maximum_filter1d(ct_arr, size=MIP_SLAB_SIZE, axis=0)[local_z]  # (Y, X)
    coronal_mip  = maximum_filter1d(ct_arr, size=MIP_SLAB_SIZE, axis=1)[local_z]  # (Y, X)
    sagittal_mip = maximum_filter1d(ct_arr, size=MIP_SLAB_SIZE, axis=2)[local_z]  # (Y, X)

    # axial ROI: z slab OR projection (percentile 기준 분모 확장용)
    half = MIP_SLAB_SIZE // 2
    z_start = max(0, local_z - half)
    z_end = min(n_z, local_z + half)
    roi_slab = np.array(roi_vol[z_start:z_end])
    roi_proj = np.any(roi_slab > 0, axis=0).astype(bool)  # (Y, X)

    # target slice ROI: 최종 mask 제한 기준
    roi_z = np.array(roi_vol[local_z]).astype(bool)  # (Y, X)

    axial_sr    = f"z{z_start}~z{z_end-1}({z_end - z_start}sl)"
    coronal_sr  = f"y_localMIP3D(axis=1,size={MIP_SLAB_SIZE})"
    sagittal_sr = f"x_localMIP3D(axis=2,size={MIP_SLAB_SIZE})"

    return (axial_mip, coronal_mip, sagittal_mip,
            roi_proj, roi_z,
            axial_sr, coronal_sr, sagittal_sr,
            z_start, z_end)


def get_lesion_masks(lesion_vol, local_z, z_start, z_end):
    """
    lesion_z  : target slice lesion (coronal/sagittal mask 제외 기준)
    lesion_proj: z slab lesion OR projection (axial mask 제외 보조)
    """
    if lesion_vol is None:
        return None, None
    lesion_z = np.array(lesion_vol[local_z]).astype(bool)          # (Y, X)
    slab = np.array(lesion_vol[z_start:z_end])
    lesion_proj = np.any(slab > 0, axis=0).astype(bool)             # (Y, X)
    return lesion_z, lesion_proj
...
def compute_all_9_masks(axial_mip, coronal_mip, sagittal_mip,
    ...
    """
    9개 mask: {plane}_{method}, 모두 (Y, X), broadcast 미사용
    axial:    roi_proj 사용 (z slab projection), lesion_proj 제외, 최종 & roi_z
    coronal:  roi_z 사용, lesion_z 제외 (최종 roi_z 내부 보장)
    sagittal: roi_z 사용, lesion_z 제외 (최종 roi_z 내부 보장)
    """
    masks = {}
    for method in ("intensity", "tophat", "vesselness"):
        # axial
        m_ax = apply_cleanup(compute_vessel_mask(axial_mip, roi_proj, lesion_proj, method))
        m_ax = m_ax & roi_z  # 최종 roi_z 내부 제한
        masks[f"axial_{method}"] = m_ax

        # coronal
        m_cor = apply_cleanup(compute_vessel_mask(coronal_mip, roi_z, lesion_z, method))
        masks[f"coronal_{method}"] = m_cor

        # sagittal
        m_sag = apply_cleanup(compute_vessel_mask(sagittal_mip, roi_z, lesion_z, method))
        masks[f"sagittal_{method}"] = m_sag
...
def compute_slice_maps(ct_vol, roi_vol, lesion_vol, local_z):
    ...
    les_z  = np.array(lesion_vol[local_z]).astype(bool) if lesion_vol is not None else None
...
def compute_candidate_table_rows(pid, local_z,
                                  ...
                                  top_patch, eligible):
    rows = []
    # axial: roi_proj/lesion_proj 사용
    # coronal/sagittal: roi_z/lesion_z 사용
    mip_map = {
        "axial":    (axial_mip,    roi_proj,  lesion_proj),
        ...
        for method in ("intensity", "tophat", "vesselness"):
            ...
            for pct in CAND_PERCENTILES:
                if vals.size > 0:
                    ...
                    if plane == "axial":
                        mask = mask & roi_z  # axial도 최종 roi_z 제한
...
def generate_png(patient, local_z, ct_vol,
    ...
    # 2: axial local MIP + 3방식 (roi_proj contour)
    base(ax[1], ax_disp, roi_proj, les_for_slice,
         f"2. Axial localMIP\n{axial_sr}")
    ...
    # 3: coronal local MIP + 3방식 (roi_z contour)
    base(ax[2], cor_disp, roi_z, les_for_slice,
         f"3. Coronal localMIP\n{coronal_sr}")
    ...
    # 4: sagittal local MIP + 3방식 (roi_z contour)
    base(ax[3], sag_disp, roi_z, les_for_slice,
         f"4. Sagittal localMIP\n{sagittal_sr}")
    ...
    # 9: corrected_v3 + corrected_v2 비교
    ...
    ax[8].legend(handles=legend_elems, loc="lower right", fontsize=5, framealpha=0.8,
                 bbox_to_anchor=(1.0, -0.28))

    short = (pid[:28] + "...") if len(pid) > 28 else pid
    fig.suptitle(
        f"{short}  |  {role}  |  z={local_z}  |  B1-E12 3D localMIP Fusion",
        fontsize=9, y=1.01
    )
    plt.tight_layout()
    png_name = f"{_STEM}_{pid_safe}_z{local_z:04d}_triplanar.png"
...
def write_report(index_rows, out_root, n_errors):
    ...
    lines = [
        "# B1-E12 3축 localMIP + Single-slice Correction 보고서",
        "",
        "## 1. 3축 local MIP 구현 방식",
        f"axial:    maximum_filter1d(ct_arr, size={MIP_SLAB_SIZE}, axis=0)[local_z] → (Y,X)",
        f"coronal:  maximum_filter1d(ct_arr, size={MIP_SLAB_SIZE}, axis=1)[local_z] → (Y,X)",
        f"sagittal: maximum_filter1d(ct_arr, size={MIP_SLAB_SIZE}, axis=2)[local_z] → (Y,X)",
        "broadcast 미사용, 모든 결과 (Y,X) 유지.",
        "최종 mask: 반드시 refined_roi[local_z] 내부로 제한.",
        "",
        "## 2. 기존 b1e12a 방식과의 차이",
        "b1e12a: coronal/sagittal = maximum_filter1d on single axial slice (2D, 개념 동일하나 명시적 3D 아님).",
        "b1e12:  coronal/sagittal = maximum_filter1d on full 3D volume, target z 추출 (3D 명시적 구현).",
        "수학적으로 동일하나 ROI 처리와 lesion 제외 기준이 개선됨.",
        "",
        "## 3. ROI / lesion 처리",
        "axial roi:    z slab OR projection → percentile 분모 확장, 최종 & roi_z.",
        "coronal roi:  roi_z (target slice ROI) 직접 사용.",
        "sagittal roi: roi_z (target slice ROI) 직접 사용.",
        "lesion 제외: axial=lesion_proj(slab), coronal/sagittal=lesion_z(target slice).",
        "",
        "## 4. pixel count 평균",
        f"full_union 평균:    {fu} px",
        f"final_fused_v2 평균: {fv2} px",
        f"final_fused_v3 평균: {fv3} px",
        f"corrected_v2 평균:  {cv2} px",
        "",
        "## 5. 최종 추천",
        "recommended_final_mask = corrected_v2",
        "이유: vote≥2로 안정적, single-slice confirm+expand로 MIP 과연결 감소.",
        "",
        "## 6. argmax 검증 (optional, v1 미구현)",
        "axial_argmax_z / coronal_argmax_y / sagittal_argmax_x 는 v1에서 미계산.",
        "필요시 v2에서 추가 예정.",
        "",
        "## 통계",
    ...
    rpath = out_root / f"{_STEM}_report.md"
...
def main():
    print("[B1-E12] 3축 local MIP + Single-slice Correction")
    ...
    if not ALLOW_REAL_PROCESSING:
        ...
        print()
        print("3축 local MIP 구현 (broadcast 미사용, 모두 (Y,X)):")
        print(f"  axial:    maximum_filter1d(ct_arr, size={MIP_SLAB_SIZE}, axis=0)[local_z] → (Y,X)")
        print(f"  coronal:  maximum_filter1d(ct_arr, size={MIP_SLAB_SIZE}, axis=1)[local_z] → (Y,X)")
        print(f"  sagittal: maximum_filter1d(ct_arr, size={MIP_SLAB_SIZE}, axis=2)[local_z] → (Y,X)")
        print("  최종 mask: 반드시 refined_roi[local_z] 내부 제한")
        print()
        print("ROI 처리:")
        print("  axial:    roi_proj (z slab OR proj) → percentile 분모, 최종 & roi_z")
        print("  coronal:  roi_z (target slice) 직접 사용")
        print("  sagittal: roi_z (target slice) 직접 사용")
        print()
        print("lesion 제외:")
        print("  axial:    lesion_proj (z slab OR projection)")
        print("  coronal:  lesion_z (target slice)")
        print("  sagittal: lesion_z (target slice)")
        print()
        print("Fusion 구조:")
        ...
        print()
        print("검증 항목:")
        print("  [O] axial/coronal/sagittal output shape 모두 (Y,X)")
        print("  [O] broadcast 미사용")
        print("  [O] 최종 mask: refined_roi[local_z] 내부 제한")
        print("  [O] stage2_holdout 교집합: 0")
        print("  [O] output root 없음")
        print("  [O] 기존 파일 수정 없음 (b1e12a로 rename 완료)")
        print()
        print("생성 예정 환자/슬라이스:")
        for p in PATIENTS:
        ...
        print()
        est_min = total_slices * 30
        est_max = total_slices * 60
        print(f"예상 PNG 수:          {total_slices}")
        print(f"예상 실행시간:        ~{est_min}~{est_max}초 (CPU, 3D volume 로드 + vesselness)")
        print(f"메모리 위험:          ~200~500MB/환자 (3D float32 로드), 순차 처리로 관리")
        print()
        print("[DRY-RUN 완료] 실행 승인 후: python b1e12_triplanar_local_mip_single_slice_correction.py --real")
        return
    ...
    for patient in PATIENTS:
        ...
        for local_z in patient["target_slices"]:
            if local_z < 0 or local_z >= n_slices:
                ...
                continue
            try:
                # 3축 local MIP (3D volume 한 번 로드, axis별 sliding)
                (axial_mip, coronal_mip, sagittal_mip,
                 roi_proj, roi_z,
                 axial_sr, coronal_sr, sagittal_sr,
                 z_start, z_end) = compute_triplanar_local_mips(ct_vol, roi_vol, local_z)

                # lesion masks
                lesion_z, lesion_proj = get_lesion_masks(lesion_vol, local_z, z_start, z_end)
                ...
                print(
                    ...
                    f"  cor_i={int(masks['coronal_intensity'].sum()):5d}"
                    f"  sag_i={int(masks['sagittal_intensity'].sum()):5d}"
    ...
    if cand_rows:
        ct_path2 = OUT_ROOT / f"{_STEM}_candidate_table.csv"
        with open(ct_path2, "w", newline="", encoding="utf-8") as f:
            w = csv.DictWriter(f, fieldnames=list(cand_rows[0].keys()))
            w.writeheader(); w.writerows(cand_rows)
        print(f"  candidate table: {ct_path2.name} ({len(cand_rows)}행)")
    ...
    summary = {
        ...
        "mip_impl": {
            "axial":    f"maximum_filter1d(ct_arr, size={MIP_SLAB_SIZE}, axis=0)[local_z]",
            "coronal":  f"maximum_filter1d(ct_arr, size={MIP_SLAB_SIZE}, axis=1)[local_z]",
            "sagittal": f"maximum_filter1d(ct_arr, size={MIP_SLAB_SIZE}, axis=2)[local_z]",
        },
        "broadcast_used": False,
        "output_shape_all": "(Y, X)",
        "final_mask_roi_constraint": "refined_roi[local_z]",
        ...
        "argmax_validation": "optional_not_computed_v1",
