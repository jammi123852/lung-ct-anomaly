"""
p_c_normal24j_fix_zroi_scalar_fusion_full_train.py

P-C-NORMAL24j-fix: z/ROI scalar-fusion full training (train/val only)
...
입력:
  - train manifest : p_c_normal24g_fix_train_feature_manifest_usable.csv  (19,716 rows)
  - val manifest   : p_c_normal24g_fix_val_feature_manifest_usable.csv    (5,189 rows)
  - scalar stats   : p_c_normal24h_fix_scalar_normalization_stats.json

출력:
  - outputs/p_c_normal24j_fix_zroi_scalar_fusion_full_train/
  - outputs/reports/p_c_normal24j_fix_zroi_scalar_fusion_full_train/
...
실행:
  python p_c_normal24j_fix_zroi_scalar_fusion_full_train.py \\
      --train \\
      --confirm-train \\
...
OUTPUT_ROOT  = PROJECT_ROOT / "outputs/p_c_normal24j_fix_zroi_scalar_fusion_full_train"
CKPT_DIR     = OUTPUT_ROOT / "checkpoints"
REPORT_ROOT  = PROJECT_ROOT / "outputs/reports/p_c_normal24j_fix_zroi_scalar_fusion_full_train"
...
def run_full_train(args) -> int:
    ...
    best_ckpt = CKPT_DIR / "p_c_normal24j_fix_best_val_auc_checkpoint.pt"
    last_ckpt = CKPT_DIR / "p_c_normal24j_fix_last_checkpoint.pt"
    ...
    if not SCALAR_STATS_PATH.exists():
        ...
        _write_csv(errors, REPORT_ROOT / "p_c_normal24j_fix_errors.csv")
        sys.exit(1)
    ...
    old_24g_used = (
        "p_c_normal24g_zroi_only_feature_manifest" in str(TRAIN_MANIFEST) and
        "_fix_" not in str(TRAIN_MANIFEST)
    )
    if old_24g_used:
    ...
    # ── 8. 결과 파일 작성 ────────────────────────────────────────────────────
    _write_csv(train_log_rows,    REPORT_ROOT / "p_c_normal24j_fix_train_log.csv")
    _write_csv(val_log_rows,      REPORT_ROOT / "p_c_normal24j_fix_val_log.csv")
    _write_csv(epoch_metric_rows, REPORT_ROOT / "p_c_normal24j_fix_epoch_metrics.csv")
    ...
    _write_json(best_ckpt_summary, REPORT_ROOT / "p_c_normal24j_fix_best_checkpoint_summary.json")
    ...
    _write_csv(guardrail_rows, REPORT_ROOT / "p_c_normal24j_fix_guardrail_check.csv")
    ...
    _write_csv(errors_to_write, REPORT_ROOT / "p_c_normal24j_fix_errors.csv")

    # training summary JSON
    training_summary = {
        "branch":             "P-C-NORMAL24j-fix-zroi-scalar-fusion-full-train",
        "step":               "full_training",
        "verdict":            verdict,
    ...
    _write_json(training_summary, REPORT_ROOT / "p_c_normal24j_fix_training_summary.json")
...
forbidden_diagnostic_wording_count={forbidden_cnt}
"""
    (REPORT_ROOT / "p_c_normal24j_fix_training_report.md").write_text(md, encoding="utf-8")

    _write_json(
        {"step": "p_c_normal24j_fix_full_train", "verdict": verdict, "timestamp": ts,
         "best_epoch": best_epoch,
         "best_val_auc": f"{best_val_auc:.4f}" if not math.isinf(best_val_auc) else "N/A",
