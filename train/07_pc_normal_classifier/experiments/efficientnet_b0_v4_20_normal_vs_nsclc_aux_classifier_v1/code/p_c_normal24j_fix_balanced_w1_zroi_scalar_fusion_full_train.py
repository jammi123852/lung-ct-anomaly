"""
p_c_normal24j_fix_balanced_w1_zroi_scalar_fusion_full_train.py

P-C-NORMAL24j-fix-balanced-w1: z/ROI scalar-fusion full training (train/val only)
이 실험은 balanced downsampling + sample_weight=1.0 reset ablation이다.
...
전제 조건:
  ...
  - p_c_normal24g_fix_balanced_w1_manifest_gen.py : balanced-w1 manifest 생성 (PASS 필요)

입력:
  - train manifest : p_c_normal24g_fix_balanced_w1_train_manifest.csv  (15,782 rows, 1:1 balanced, sw=1.0)
  - val manifest   : p_c_normal24g_fix_balanced_w1_val_manifest.csv    (4,160 rows, 1:1 balanced, sw=1.0)
  - scalar stats   : p_c_normal24h_fix_scalar_normalization_stats.json

출력:
  - outputs/p_c_normal24j_fix_balanced_w1_zroi_scalar_fusion_full_train/
  - outputs/reports/p_c_normal24j_fix_balanced_w1_zroi_scalar_fusion_full_train/
...
실행:
  python p_c_normal24j_fix_balanced_w1_zroi_scalar_fusion_full_train.py \\
      --train \\
      --confirm-train \\
...
OUTPUT_ROOT  = PROJECT_ROOT / "outputs/p_c_normal24j_fix_balanced_w1_zroi_scalar_fusion_full_train"
CKPT_DIR     = OUTPUT_ROOT / "checkpoints"
REPORT_ROOT  = PROJECT_ROOT / "outputs/reports/p_c_normal24j_fix_balanced_w1_zroi_scalar_fusion_full_train"
...
def run_full_train(args) -> int:
    ...
    stage_label  = "P-C-NORMAL24j-fix-balanced-w1"
    ...
    best_ckpt = CKPT_DIR / "p_c_normal24j_fix_balanced_w1_best_val_auc_checkpoint.pt"
    last_ckpt = CKPT_DIR / "p_c_normal24j_fix_balanced_w1_last_checkpoint.pt"
    ...
    if not SCALAR_STATS_PATH.exists():
        ...
        _write_csv(errors, REPORT_ROOT / "p_c_normal24j_fix_balanced_w1_errors.csv")
        sys.exit(1)
    ...
    for split_name, df in [("train", df_train_raw), ("val", df_val_raw)]:
        if "sample_weight" in df.columns:
            ...
            if not (sw_vals == 1.0).all():
                errors.append({"check": f"{split_name}_sw_reset", "error": "sample_weight is not all 1.0 — use balanced-w1 manifest"})
    ...
    old_24g_used = (
        "p_c_normal24g_fix_balanced_w1_feature_manifest" not in str(TRAIN_MANIFEST)
    )
    if old_24g_used:
        errors.append({"check": "old_24g_guard", "error": "balanced_w1 manifest not detected in path"})
        sys.exit(1)
    ...
    # ── 8. 결과 파일 작성 ────────────────────────────────────────────────────
    _write_csv(train_log_rows,    REPORT_ROOT / "p_c_normal24j_fix_balanced_w1_train_log.csv")
    _write_csv(val_log_rows,      REPORT_ROOT / "p_c_normal24j_fix_balanced_w1_val_log.csv")
    _write_csv(epoch_metric_rows, REPORT_ROOT / "p_c_normal24j_fix_balanced_w1_epoch_metrics.csv")
    ...
    best_ckpt_summary = {
        ...
        "sample_weight_reset_to_1": True,
        "original_sample_weight_preserved": True,
        "note": "val 기준 선택 checkpoint — final_test 결과 아님. balanced-w1 ablation 전용.",
    }
    _write_json(best_ckpt_summary, REPORT_ROOT / "p_c_normal24j_fix_balanced_w1_best_checkpoint_summary.json")
    ...
    guardrail_rows = [
        ...
        {"guardrail": "balanced_w1_ablation",                "expected": True,  "actual": True,  "pass": True},
        {"guardrail": "balanced_train_val_only",             "expected": True,  "actual": True,  "pass": True},
        {"guardrail": "sample_weight_reset_to_1",            "expected": True,  "actual": True,  "pass": True},
        {"guardrail": "original_sample_weight_preserved",    "expected": True,  "actual": True,  "pass": True},
    ]
    _write_csv(guardrail_rows, REPORT_ROOT / "p_c_normal24j_fix_balanced_w1_guardrail_check.csv")
    ...
    _write_csv(errors_to_write, REPORT_ROOT / "p_c_normal24j_fix_balanced_w1_errors.csv")

    # training summary JSON
    training_summary = {
        "branch":             "P-C-NORMAL24j-fix-balanced-w1-zroi-scalar-fusion-full-train",
        "step":               "full_training",
        "verdict":            verdict,
    ...
    _write_json(training_summary, REPORT_ROOT / "p_c_normal24j_fix_balanced_w1_training_summary.json")
    ...
    md = f"""# P-C-NORMAL24j-fix-balanced-w1 z/ROI Scalar-Fusion Full Training 결과
...
> **주의**: 이 실험은 balanced downsampling + sample_weight=1.0 reset ablation이다.
> main 24j-fix를 자동으로 대체하지 않는다.
> 이번 결과는 train/val 기준이다. final_test 성능이 아니다.
> val AUROC/AUPRC는 checkpoint 선택 및 학습 안정성 확인용이다.
> final_test 성능 비교는 사용자 승인 후 별도 단계에서만 수행한다.
...
| Loss | BCEWithLogitsLoss(reduction=none) × sample_weight(=1.0) → mean |
...
|---|---|---|---|
| train (balanced-w1) | {n_train} | {n0_tr} | {n1_tr} |
| val (balanced-w1)   | {n_val}   | {n0_vl} | {n1_vl} |

- balanced-w1 manifest 사용: True (sample_weight=1.0)
...
forbidden_diagnostic_wording_count={forbidden_cnt}
"""
    (REPORT_ROOT / "p_c_normal24j_fix_balanced_w1_training_report.md").write_text(md, encoding="utf-8")

    _write_json(
        {"step": "p_c_normal24j_fix_balanced_w1_full_train", "verdict": verdict, "timestamp": ts,
         "best_epoch": best_epoch,
         "best_val_auc": f"{best_val_auc:.4f}" if not math.isinf(best_val_auc) else "N/A",
    ...
    print(f"  [주의] 이 결과는 balanced-w1 ablation. train/val 기준. final_test 성능 아님.")
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(description="P-C-NORMAL24j-fix-balanced-w1 z/ROI scalar-fusion full train")
