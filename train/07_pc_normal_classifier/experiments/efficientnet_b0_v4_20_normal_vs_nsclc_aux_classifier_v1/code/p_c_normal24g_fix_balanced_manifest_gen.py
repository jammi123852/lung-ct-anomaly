"""
p_c_normal24g_fix_balanced_manifest_gen.py

24g-fix usable manifest를 train/val split별로 1:1 balanced downsampling.
(final_test는 처리하지 않음 — train/val class-balanced ablation 전용)
seed=42 고정. 다수 클래스에서 무작위 샘플링.
...
출력:
  outputs/manifests/p_c_normal24g_fix_balanced_feature_manifest/
    p_c_normal24g_fix_balanced_train_manifest.csv      (15,782: 7891+7891)
    p_c_normal24g_fix_balanced_val_manifest.csv        (4,160: 2080+2080)
    p_c_normal24g_fix_balanced_manifest_summary.json
    p_c_normal24g_fix_balanced_sample_weight_check.csv
    DONE.json
...
OUT_DIR = PROJECT_ROOT / "outputs/manifests/p_c_normal24g_fix_balanced_feature_manifest"
...
OUT_NAMES = {
    "train": "p_c_normal24g_fix_balanced_train_manifest.csv",
    "val":   "p_c_normal24g_fix_balanced_val_manifest.csv",
}
...
def compute_sample_weight_stats(df: pd.DataFrame, split_name: str) -> dict:
    if "sample_weight" not in df.columns:
        return {
            ...
            "sw_all_ones": None, "verdict": "FAIL_no_sample_weight_col",
        }
...
def main():
    ...
    if sw_check_rows:
        sw_csv_path = OUT_DIR / "p_c_normal24g_fix_balanced_sample_weight_check.csv"
    ...
    with open(OUT_DIR / "p_c_normal24g_fix_balanced_manifest_summary.json", "w") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    with open(OUT_DIR / "DONE.json", "w") as f:
        json.dump({
            "step":                 "p_c_normal24g_fix_balanced_manifest_gen",
