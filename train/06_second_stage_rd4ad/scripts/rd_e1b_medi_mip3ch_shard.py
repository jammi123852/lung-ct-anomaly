"""
RD-E1b: Mediastinal window MIP3ch shard-optimized true RD4AD
목적: 실험 B — medi window [-160,240] + MIP3ch (ch0=MIP(z-3~z-1), ch1=MIP(z-1~z+1), ch2=MIP(z+1~z+3)).
...
SHARD_ROOT = (
    PROJECT_ROOT
    / "outputs/normal_based_stage2_verifier_audit"
    / "rd_e1b_medi_mip3ch_train_shards_v1"
)
MODEL_ROOT_SHARD = (
    PROJECT_ROOT / "outputs/models/rd_e1b_true_rd4ad_resnet18_medi_mip3ch_shard_v1"
)
REPORT_ROOT = (
    PROJECT_ROOT
    / "outputs/normal_based_stage2_verifier_audit"
    / "rd_e1b_medi_mip3ch_true_rd4ad_shard_v1"
)
...
def build_mip3ch_crop(ct_arr, local_z, y0, x0, y1, x1):
    ...
    ch0=MIP(z-3~z-1), ch1=MIP(z-1~z+1), ch2=MIP(z+1~z+3). Medi window.
