"""
RD-E1a: Lung window 3ch shard-optimized true RD4AD
목적: 실험 A — lung window [-1000,600] + z-1/z/z+1 단순 스택. RD-D1s와 window만 다름.
...
REPORT_ROOT = (
    PROJECT_ROOT
    / "outputs/normal_based_stage2_verifier_audit"
    / "rd_e1a2_lung3ch_roipx_true_rd4ad_shard_v1"
)
...
def apply_roi_mask_3ch(crop, mask_arr, z, y0, x0, y1, x1):
    """lung3ch crop에 채널별 ROI 마스크 적용.
    ch0=z-1, ch1=z, ch2=z+1 에 각각 해당 슬라이스 마스크 적용.
    mask_arr: (Z,H,W). None이면 그대로 반환.
    """
    if mask_arr is None:
        return crop
    import numpy as np
    ...
    crop = crop.copy()
    crop[0][~_get_roi(zm)] = 0.0  # ch0: z-1
    crop[1][~_get_roi(z)]  = 0.0  # ch1: z
    crop[2][~_get_roi(zp)] = 0.0  # ch2: z+1
    return crop.astype(np.float32)
...
def run_smoke_shard_profile():
    ...
    for idx in check_indices:
        ...
        mask_path = MASK_ROOT_NORMAL / row["safe_id"] / "refined_roi.npy"
        mask_arr2 = mask_cache2.get(mask_path) if mask_path.exists() else None
        crop_live = build_lung3ch_crop(
            ct_arr,
            int(row["local_z"]),
            int(row["crop_y0"]), int(row["crop_x0"]),
            int(row["crop_y1"]), int(row["crop_x1"]),
        )
        crop_live = apply_roi_mask_3ch(
            crop_live, mask_arr2,
            int(row["local_z"]),
            int(row["crop_y0"]), int(row["crop_x0"]),
            int(row["crop_y1"]), int(row["crop_x1"]),
