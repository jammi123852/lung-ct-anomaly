"""
RD-E1c2: Lung window MIP3ch + ROI pixel mask true RD4AD shard-based full run
목적: 실험 C2 — C와 동일하되 val/candidate scoring 시 v4_20_modeB_all 마스크 픽셀 단위 적용.
...
def run_all_shard():
    ...
    val_mask_cache = CtMmapCache(max_size=8)
    BATCH_VAL      = 64
    n_val_scored   = 0
    ...
    for safe_id, v_rows in val_groups.items():
        ...
        val_mask_path = MASK_ROOT_NORMAL / safe_id / "refined_roi.npy"
        val_mask_arr  = val_mask_cache.get(val_mask_path) if val_mask_path.exists() else None
    ...
    score_mask_cache = CtMmapCache(max_size=12)
