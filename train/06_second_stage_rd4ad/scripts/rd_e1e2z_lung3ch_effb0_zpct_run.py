"""
RD-E1e2z: Lung window 3ch EfficientNet-B0 RD4AD + lung_z_percentile 주입 실험
목적: 실험 E2z — E2와 동일 설정에서 student decoder de_late 입력에 lung_z_pct (81ch) 추가.
     z축 위치 정보가 이상탐지 성능을 개선하는지 확인.
...
SHARD_ROOT        = (
    ...
    / "rd_e1a_lung3ch_train_shards_v1"
)
...
# 출력
MODEL_ROOT = (
    PROJECT_ROOT / "outputs/models/rd_e1e2z_effb0_lung3ch_zpct_v1"
)
...
OUTPUT_ROOT = (
    PROJECT_ROOT
    / "outputs/normal_based_stage2_verifier_audit"
    / "rd_e1e2z_lung3ch_effb0_zpct_shard_run_v1"
)
...
LOCAL_WEIGHT_PATH = Path(
    "/home/jinhy/.cache/torch/hub/checkpoints/efficientnet_b0_rwightman-7f5810bc.pth"
)
...
def build_student_decoder():
    """EfficientNet-B0 대응 decoder + z_pct 주입: late(81ch)→mid(40ch)→early(24ch)
    de_late 입력: teacher late_feat(80ch) + lung_z_pct broadcast(1ch) = 81ch
    """
    ...
    class StudentDecoder(nn.Module):
        def __init__(self):
            ...
            self.de_late = nn.Sequential(
                nn.Conv2d(81, 80, 3, 1, 1),  # z_pct 1ch 추가로 81ch 입력
                nn.BatchNorm2d(80),
        ...
        def forward(self, late_feat_with_z):
            # late_feat_with_z: (B, 81, H, W) — teacher_late(80ch) + z_pct(1ch)
...
def run_dry_plan():
    ...
    print(f"  input_mode      : lung3ch_zminus1_z_zplus1_zpct")
    print(f"  window          : HU[{MEDI_HU_MIN},{MEDI_HU_MAX}] -> [0,1]")
    print(f"  model_type      : true_RD4AD_EfficientNetB0_teacher_student_zpct")
...
def run_all_shard():
    ...
    if not LOCAL_WEIGHT_PATH.exists():
        print(f"[ABORT] EfficientNet-B0 local weight 없음: {LOCAL_WEIGHT_PATH}", file=sys.stderr)
        sys.exit(1)
    ...
    # ── z_pct lookup table 빌드 (shard index 기반) ───────────────────────────
    print("\n[1.5/7] lung_z_pct lookup table 빌드")
    safe_id_z_min = {}
    safe_id_z_max = {}
    shard_lookup = {}  # (shard_id_int, offset_int) → (safe_id, local_z)
    with open(SHARD_INDEX_CSV, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            sid_val  = row["safe_id"]
            lz_val   = int(row["local_z"])
            sh_id    = int(row["shard_id"])
            off_val  = int(row["offset"])
            if sid_val not in safe_id_z_min or lz_val < safe_id_z_min[sid_val]:
                safe_id_z_min[sid_val] = lz_val
            if sid_val not in safe_id_z_max or lz_val > safe_id_z_max[sid_val]:
                safe_id_z_max[sid_val] = lz_val
            shard_lookup[(sh_id, off_val)] = (sid_val, lz_val)

    def get_z_pct_from_lookup(safe_id, local_z):
        z_min = safe_id_z_min.get(safe_id, local_z)
        z_max = safe_id_z_max.get(safe_id, local_z)
        if z_max == z_min:
            return 0.5
        return float(local_z - z_min) / float(z_max - z_min)

    print(f"  patients={len(safe_id_z_min):,}  shard_entries={len(shard_lookup):,}  OK")
    ...
    for epoch in range(EPOCHS):
        ...
        for step, batch_items in sampler.epoch_batches(epoch):
            ...
            for shard_id, offset in batch_items:
                ...
                sl_sid, sl_lz = shard_lookup.get((shard_id, offset), ("", 0))
                z_pcts.append(get_z_pct_from_lookup(sl_sid, sl_lz))
            batch_np    = np.stack(crops, axis=0)
            z_pct_np    = np.array(z_pcts, dtype=np.float32)
            ...
            # z_pct broadcast concat: (B,80,H,W) + (B,1,H,W) → (B,81,H,W)
            B_tr, _, H_tr, W_tr = tf3.shape
            z_map_tr = torch.from_numpy(z_pct_np).to(device).view(B_tr, 1, 1, 1).expand(B_tr, 1, H_tr, W_tr)
            tf3_with_z = torch.cat([tf3, z_map_tr], dim=1)

            de3, de2, de1 = student(tf3_with_z)
    ...
    # val: safe_id별 z 범위 미리 계산
    val_z_min = {}
    val_z_max = {}
    for sid_v, rows_v in val_groups.items():
        zs_v = [int(r["local_z"]) for r in rows_v]
        val_z_min[sid_v] = min(zs_v)
        val_z_max[sid_v] = max(zs_v)

    def get_val_z_pct(safe_id, local_z):
        z_min = val_z_min.get(safe_id, local_z)
        z_max = val_z_max.get(safe_id, local_z)
        if z_max == z_min:
            return 0.5
        return float(local_z - z_min) / float(z_max - z_min)
    ...
    for safe_id, v_rows in val_groups.items():
        ...
        for i in range(0, len(v_rows), BATCH_VAL):
            ...
            with torch.no_grad():
                ...
                tf2 = teacher_features["mid"]
                tf1 = teacher_features["early"]
                # z_pct concat for val scoring
                z_pcts_val = np.array(
                    [get_val_z_pct(safe_id, int(r["local_z"])) for r in batch_rows],
                    dtype=np.float32
                )
                B_v, _, H_v, W_v = tf3.shape
                z_map_v = torch.from_numpy(z_pcts_val).to(device).view(B_v, 1, 1, 1).expand(B_v, 1, H_v, W_v)
                tf3_with_z_v = torch.cat([tf3, z_map_v], dim=1)
                de3, de2, de1 = student(tf3_with_z_v)
    ...
    threshold_summary = {
        ...
        "model_tag":              "rd_e1e2z_true_rd4ad_effb0_lung3ch_zpct_v1",
    ...
    with open(OUTPUT_ROOT / "rd_e1e2z_normal_val_threshold_summary.json", "w", encoding="utf-8") as f:
        json.dump(threshold_summary, f, indent=2)
    print(f"  saved: rd_e1e2z_normal_val_threshold_summary.json")
    ...
    # candidate: safe_id별 z 범위 미리 계산
    c2_z_min = {}
    c2_z_max = {}
    for sid_c, rows_c in c2_groups.items():
        zs_c = [int(r["local_z"]) for r in rows_c]
        c2_z_min[sid_c] = min(zs_c)
        c2_z_max[sid_c] = max(zs_c)

    def get_c2_z_pct(safe_id, local_z):
        z_min = c2_z_min.get(safe_id, local_z)
        z_max = c2_z_max.get(safe_id, local_z)
        if z_max == z_min:
            return 0.5
        return float(local_z - z_min) / float(z_max - z_min)
    ...
    for safe_id, rows_for_pat in c2_groups.items():
        ...
        for i in range(0, len(rows_for_pat), BATCH_SCORE):
            ...
            with torch.no_grad():
                ...
                tf2 = teacher_features["mid"]
                tf1 = teacher_features["early"]
                # z_pct concat for candidate scoring
                z_pcts_sc = np.array(
                    [get_c2_z_pct(safe_id, int(r["local_z"])) for r in batch_rows],
                    dtype=np.float32
                )
                B_sc, _, H_sc, W_sc = tf3.shape
                z_map_sc = torch.from_numpy(z_pcts_sc).to(device).view(B_sc, 1, 1, 1).expand(B_sc, 1, H_sc, W_sc)
                tf3_with_z_sc = torch.cat([tf3, z_map_sc], dim=1)
                de3, de2, de1 = student(tf3_with_z_sc)
            ...
            for j, row in enumerate(batch_rows):
                ...
                score_rows.append({
                    ...
                    "rd_e1e2z_lung3ch_zpct_rd4ad_score": round(rd4ad_score, 6),
    ...
    score_fieldnames = [
        ...
        "rd_e1e2z_lung3ch_zpct_rd4ad_score",
        "global_p95_exceed", "global_p99_exceed",
        "bin_p95_exceed", "bin_p99_exceed",
        "score_nan", "score_inf",
    ]
    write_csv(
        OUTPUT_ROOT / "rd_e1e2z_zpct_candidate_score.csv",
    ...
    y_score = np.array(
        [float(r["rd_e1e2z_lung3ch_zpct_rd4ad_score"]) for r in score_rows], dtype=float
    )
        ...
        for r in score_rows:
            if r["label"] == "positive":
                by_pat[r["patient_id"]].append(
                    float(r["rd_e1e2z_lung3ch_zpct_rd4ad_score"]) <= thr
                )
    ...
    for r in score_rows:
        ...
        by_pat_all[r["patient_id"]]["scores"].append(
            float(r["rd_e1e2z_lung3ch_zpct_rd4ad_score"])
        )
    write_csv(
    ...
    write_csv(
        ...
        [
            {"experiment": "RD-B8f", "model_type": "true_RD4AD_EfficientNetB0",
    ...
    # ── summary JSON ─────────────────────────────────────────────────────────
    summary = {
        "input_mode":                          "lung3ch_zminus1_z_zplus1_zpct",
        "train_source":                        "prebuilt_float32_shards",
        "model_type":                          "true_RD4AD_EfficientNetB0_teacher_student_zpct",
    ...
    with open(OUTPUT_ROOT / "rd_e1e2z_lung3ch_zpct_shard_run_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, default=str)
    print("  saved: rd_e1e2z_lung3ch_zpct_shard_run_summary.json")
    ...
    md_lines = [
        ...
        "| 항목 | 값 |",
        "|---|---|",
        f"| input_mode | lung3ch_zminus1_z_zplus1_zpct |",
    ...
    with open(OUTPUT_ROOT / "rd_e1e2z_lung3ch_zpct_shard_run_report.md", "w", encoding="utf-8") as f:
        f.write("\n".join(md_lines) + "\n")
    print("  saved: rd_e1e2z_lung3ch_zpct_shard_run_report.md")
