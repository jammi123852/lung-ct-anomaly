"""
RD-E1c2: Lung window MIP3ch + ROI pixel mask shard-optimized true RD4AD
목적: 실험 C2 — C와 동일하되 shard 빌드 시 v4_25_modeB_train 마스크를 픽셀 단위로 적용
     (ROI 외부 흉벽 픽셀 = 0). C 결과와 비교해서 픽셀 마스킹 효과 검증.
...
SHARD_ROOT = (
    PROJECT_ROOT
    / "outputs/normal_based_stage2_verifier_audit"
    / "rd_e1c2_lung_mip3ch_roipx_train_shards_v1"
)
MODEL_ROOT_SHARD = (
    PROJECT_ROOT / "outputs/models/rd_e1c2_true_rd4ad_resnet18_lung_mip3ch_roipx_shard_v1"
)
REPORT_ROOT = (
    PROJECT_ROOT
    / "outputs/normal_based_stage2_verifier_audit"
    / "rd_e1c2_lung_mip3ch_roipx_true_rd4ad_shard_v1"
)

# ROI mask (정상 학습 데이터용 - v4_25_modeB_train)
MASK_ROOT_NORMAL = (
    PROJECT_ROOT
...
def build_mip3ch_crop(ct_arr, local_z, y0, x0, y1, x1):
    ...
    ch0=MIP(z-3~z-1), ch1=MIP(z-1~z+1), ch2=MIP(z+1~z+3). Lung window.
    OOB → reflect padding (edge fallback if valid region <= 1px).
    """
    ...
    valid_h = cy1 - cy0; valid_w = cx1 - cx0
    if needs_pad:
    ...
    def _build_ch(z_idx):
        zi = int(np.clip(z_idx, 0, Z - 1))
        normed = _win(ct_arr[zi, cy0:cy1, cx0:cx1])
        if needs_pad:
    ...
    def _mip_ch(z_list):
        return np.stack([_build_ch(zi) for zi in z_list], axis=0).max(axis=0)

    ch0 = _mip_ch([z - 3, z - 2, z - 1])
    ch1 = _mip_ch([z - 1, z,     z + 1])
    ch2 = _mip_ch([z + 1, z + 2, z + 3])
...
def apply_roi_mask(crop, mask_arr, z, y0, x0, y1, x1):
    """(3,96,96) crop에 ROI 마스크 적용 - ROI 외부 픽셀을 0.0으로 설정.
    mask_arr: (Z,H,W) bool/uint8. None이면 crop을 그대로 반환.
    """
    if mask_arr is None:
        return crop
    import numpy as np
    Z, H, W = mask_arr.shape
    z_clamped = int(np.clip(z, 0, Z - 1))
    cy0 = max(0, y0); cy1 = min(H, y1)
    cx0 = max(0, x0); cx1 = min(W, x1)
    roi = mask_arr[z_clamped, cy0:cy1, cx0:cx1].astype(bool)
    pad_top    = max(0, -y0)
    pad_bottom = max(0, y1 - H)
    pad_left   = max(0, -x0)
    pad_right  = max(0, x1 - W)
...
def run_smoke_shard_profile():
    ...
    mask_cache = CtMmapCache(max_size=48)
    ...
    for i, row in enumerate(smoke_manifest_rows):
        ...
        mask_path = MASK_ROOT_NORMAL / row["safe_id"] / "refined_roi.npy"
        mask_arr  = mask_cache.get(mask_path) if mask_path.exists() else None

        _ba = _G_PAD_APPLIED_COUNT
        _br = _G_PAD_REFLECT_COUNT
        _be = _G_PAD_EDGE_COUNT
        try:
            crop = build_mip3ch_crop(
                ct_arr,
                int(row["local_z"]),
    ...
    mask_cache2  = CtMmapCache(max_size=48)
    max_abs_diff = 0.0
    ...
    for idx in check_indices:
        ...
        mask_path = MASK_ROOT_NORMAL / row["safe_id"] / "refined_roi.npy"
        mask_arr  = mask_cache2.get(mask_path) if mask_path.exists() else None
        crop_live = build_mip3ch_crop(
            ct_arr,
            int(row["local_z"]),
            int(row["crop_y0"]), int(row["crop_x0"]),
            int(row["crop_y1"]), int(row["crop_x1"]),
        )
        crop_live = apply_roi_mask(
            crop_live, mask_arr,
            int(row["local_z"]),
            int(row["crop_y0"]), int(row["crop_x0"]),
            int(row["crop_y1"]), int(row["crop_x1"]),
