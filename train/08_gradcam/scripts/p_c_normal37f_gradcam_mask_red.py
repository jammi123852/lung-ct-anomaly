"""
P-C-NORMAL37f: Grad-CAM mask-inside-only red overlay
- 기존 37e와 동일한 샘플/체크포인트/입력 로딩
- heatmap을 mask 내부로만 제한 (cam_masked = cam * mask_96)
- mask 내부 픽셀만으로 min-max normalize
- "위험한 부분" = 빨간색, 낮은 값 = 투명
- 6-panel PNG (raw / masked / mask / masked+red / raw+boundary+red / heatmap-only)
"""
...
OUTPUT_DIR   = PROJECT_ROOT / "outputs/p_c_normal37f_gradcam_mask_inside_red_overlay"
REPORT_DIR   = PROJECT_ROOT / "outputs/reports/p_c_normal37f_gradcam_mask_inside_red_overlay"
...
# Yellow→Orange→Red gradient: 낮은 값=투명, 높은 값=진한 빨강
HEAT_CMAP = LinearSegmentedColormap.from_list(
    "heat_yorr",
    [(0.00, (1.0, 1.0, 0.6, 0.00)),   # 투명 (near-zero)
     (0.10, (1.0, 1.0, 0.2, 0.12)),   # light yellow, 약한 alpha
     (0.35, (1.0, 0.85, 0.0, 0.38)),  # yellow
     (0.60, (1.0, 0.50, 0.0, 0.62)),  # orange
     (0.80, (1.0, 0.18, 0.0, 0.80)),  # orange-red
     (1.00, (0.88, 0.0, 0.0, 0.95))], # deep red (hotspot)
    N=256,
)
...
def compute_gradcam_raw(model, img_t, sc_t):
    """
    model.img_features[7] → backward → GAP weights → cam (96×96, 0~1 before masking)
    returns: cam_np_raw (96×96), logit_val, prob_val
    """
    activations = {}
    gradients   = {}
...
def apply_mask_inside_norm(cam_raw, mask_96):
    """
    1) cam * mask_96 → mask 바깥 = 0
    2) mask 내부 픽셀만으로 min-max normalize
    3) mask 내부 값이 모두 같으면 → zero_cam=True
    returns: cam_masked (96×96), zero_cam_flag
    """
...
def prep_input(row, scalar_stats):
    ...
    ct_crop = data["ct_crop"].astype(np.float32)        # (3, 96, 96) HU
...
def make_heat_rgba(cam_norm):
    """cam_norm [0,1] → RGBA (yellow→orange→red gradient, alpha ∝ score)"""
    return HEAT_CMAP(cam_norm).astype(np.float32)  # (H, W, 4)
...
def save_panel_png(raw_disp, masked_disp, mask_96, cam_norm, zero_cam,
    ...
    axes[1, 1].set_title("5. raw + mask boundary + Grad-CAM (red)", fontsize=7, color="white")
    axes[1, 1].axis("off")

    # [1,2] heatmap only (검정 배경에 빨강만)
    ...
    if not zero_cam:
        ...
        axes[1, 2].imshow(np.zeros_like(raw_disp), cmap="gray", vmin=0, vmax=1)
    ...
    cbar.set_label("anomaly score (yellow=low → red=high)", color="white", fontsize=6)
    cbar.ax.yaxis.set_tick_params(color="white")
    plt.setp(cbar.ax.yaxis.get_ticklabels(), color="white")
    axes[1, 2].set_title("6. Grad-CAM only (red=danger)", fontsize=7, color="white")
    axes[1, 2].axis("off")
...
def save_context_png(row, cam_norm, png_path, plt):
    ...
    axes[0].set_title(f"full CT slice z={z_str}  (red box=crop)", fontsize=7, color="white")
    axes[0].axis("off")
    ...
    heat_rgba = make_heat_rgba(cam_norm[:crop_zoom.shape[0], :crop_zoom.shape[1]])
    axes[1].imshow(crop_zoom, cmap="gray", vmin=0, vmax=1)
    axes[1].imshow(heat_rgba)
    axes[1].set_title("crop zoom + Grad-CAM (red=danger)", fontsize=7, color="white")
    axes[1].axis("off")
...
def write_report(sample_metrics, report_dir):
    lines = [
        "# P-C-NORMAL37f Grad-CAM Mask-Inside Red Overlay Report",
        "",
        "## 1. 이번 수정 목적",
        "- 기존 37e heatmap이 mask 외부까지 번지고, 색이 직관적이지 않은 문제를 개선",
        "- mask 내부로만 Grad-CAM을 제한 → `cam_masked = cam * mask_96`",
        "- mask 내부 픽셀만 min-max normalize → 상대적 위험도가 0~1로 표현됨",
        "- 위험한 부분을 빨간색(alpha ∝ 값)으로 표시",
        "",
        "## 2. 기존 37e와의 차이",
        "| 항목 | 37e | 37f |",
        "|---|---|---|",
        "| heatmap 범위 | 96×96 전체 | mask 내부만 |",
        "| normalize 기준 | 96×96 전체 픽셀 | mask 내부 픽셀만 |",
        "| 컬러맵 | hot (흰색=최고) | red alpha overlay (빨강=위험) |",
        "| 배경 | 밝은 배경 | 어두운 배경 (#111) |",
        "",
        "## 3. 판정 기준",
        "- **near_blank**: low_mask=True 또는 mask_ratio < 0.10",
        "- **lesion_focused**: peak_inside_mask=True, inside_cam_mean > outside_cam_mean, in_std >= 0.15",
        "- **diffuse_inside_mask**: 위 조건 충족하나 in_std < 0.15 (mask 내 전반에 분산)",
    ...
    for m in nsclc:
        lines.append(f"- **{m['patient_id']}** z={m['canonical_volume_z']}: `{m['tag']}` "
                     f"(inside_mean={m['inside_cam_mean']:.3f}, ratio={m['inside_to_outside_ratio']})")

    lines.append("")
    lines.append("### Normal FP")
    for m in fp:
        lines.append(f"- **{m['patient_id'][:30]}** z={m['canonical_volume_z']}: `{m['tag']}` "
                     f"(peak_in_mask={m['peak_inside_mask']}, inside_mean={m['inside_cam_mean']:.3f})")

    lines.append("")
    lines.append("### low_mask FN")
    for m in lm:
        lines.append(f"- **{m['patient_id']}** z={m['canonical_volume_z']}: `{m['tag']}` "
    ...
    lines += [
        "",
        "## 6. Caveat",
        "- 이 시각화는 **mask-inside-only visualization**이다.",
        "- \"모델이 mask 바깥을 봤는지\"를 논하는 것이 아니다.",
        "- masked-input 모델의 입력 공간(mask 내부) 안에서 어떤 위치에",
        "  더 높은 gradient activation이 집중됐는지를 보여주는 것이다.",
        "- 색이 진한 빨강 = gradient가 집중된 위치 = 모델이 판단에 더 활용한 구조",
...
def main():
    ...
    guardrail_rows = [
        ...
        {"item": "no_37e_overwrite",                "value": True},
    ...
    summary = {
        ...
        "colormap": "red alpha overlay (low=transparent, high=red)",
        "no_training": True,
