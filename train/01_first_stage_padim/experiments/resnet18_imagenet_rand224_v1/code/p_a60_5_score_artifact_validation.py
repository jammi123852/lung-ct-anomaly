"""
P-A60.5: ResNet18 random224 stage1_dev score artifact validation (read-only).

- P-A60 score CSV 154개를 pandas/csv.reader로 read-only 검증.
- np.loadtxt 사용 금지.
- metrics 계산 금지, scoring/forward/training 금지.
- stage2_holdout 접근 금지.

실행:
  source ~/ai_env/bin/activate && python experiments/resnet18_imagenet_rand224_v1/code/p_a60_5_score_artifact_validation.py
"""
...
# 입력 (read-only)
# ------------------------------------------------------------------
SCORE_DIR = ws_paths.OUTPUTS / "scores" / "lesion_stage1_dev_by_patient"
P_A60_REPORT_JSON = ws_paths.OUTPUTS / "reports" / "lesion_stage1_dev" / "p_a60_lesion_stage1_dev_scoring.json"
P_A60_REPORT_MD = ws_paths.OUTPUTS / "reports" / "lesion_stage1_dev" / "p_a60_lesion_stage1_dev_scoring.md"
...
# reference: P-A20 ResNet50 rand224 score dir (row count 비교용, read-only)
REF_SCORE_DIR = REPO_ROOT / "experiments" / "resnet50_imagenet_rand224_v1" / "outputs" / "scores" / "lesion_stage1_dev_by_patient"
...
# P-A60 보고서 값 (검증 기준)
P_A60_REPORTED_PATCHES = 2_760_497
P_A60_P95_EXCEED = 832_485
P_A60_P99_EXCEED = 348_361

# 필수 컬럼
REQUIRED_COLS = {"patient_id", "slice_index", "y0", "x0", "y1", "x1",
                 "padim_score", "position_bin", "has_lesion_patch"}
OPTIONAL_COLS = {"safe_id", "label", "local_z", "z_level", "lesion_pixels", "patch_label"}

# ------------------------------------------------------------------
# 출력 경로
# ------------------------------------------------------------------
OUT_DIR = ws_paths.OUTPUTS / "reports" / "lesion_stage1_dev" / "p_a60_5_score_artifact_validation"
...
REPORT_MD = OUT_DIR / "p_a60_5_score_artifact_validation.md"
REPORT_JSON = OUT_DIR / "p_a60_5_score_artifact_validation.json"


def stop(msg: str) -> None:
    print(f"[P-A60.5][ABORT] {msg}")
    sys.exit(1)


def run_guards(stage1_dev_ids: set, holdout_ids: set) -> dict:
    # G1: P-A60 보고서 통과 확인
    if not P_A60_REPORT_MD.exists():
        stop(f"P-A60 보고서 없음: {P_A60_REPORT_MD}")
    with open(P_A60_REPORT_MD, encoding="utf-8") as f:
        md_text = f.read()
    if "판정: 통과" not in md_text:
        stop(f"P-A60 보고서가 통과 상태가 아님")
    print(f"[P-A60.5][G1] P-A60 보고서 통과 확인")

    # P-A60 JSON 로드 (read-only)
    p60 = json.load(open(P_A60_REPORT_JSON, encoding="utf-8"))

    # G2: score CSV 수 154개 확인
    csv_files = list(SCORE_DIR.glob("*.csv"))
    if len(csv_files) != EXPECTED_N:
        stop(f"score CSV 수가 {len(csv_files)} (기대 {EXPECTED_N})")
    print(f"[P-A60.5][G2] score CSV {len(csv_files)}개 확인")

    # G5: stage2_holdout contamination 0 확인
    csv_pids = {f.stem for f in csv_files}
    holdout_in_score = csv_pids & holdout_ids
    if holdout_in_score:
        stop(f"stage2_holdout 환자가 score dir에 존재: {holdout_in_score}")
    print(f"[P-A60.5][G5] stage2_holdout contamination 0 확인")

    # G11: 출력 경로 기존 결과 없음 확인
    if REPORT_JSON.exists() or REPORT_MD.exists():
        stop(f"기존 P-A60.5 결과 존재 → 덮어쓰기 금지: {OUT_DIR}")
    print(f"[P-A60.5][G11] 출력 경로 기존 결과 없음 확인")
...
def main() -> None:
    ...
    # 가드 실행
    p60 = run_guards(stage1_dev_ids, holdout_ids)
    ...
    print(f"[P-A60.5] threshold p95={p95:.6f}, p99={p99:.6f} (read-only)")

    print(f"\n[P-A60.5] 모든 가드 통과. score artifact validation 시작.")
    ...
    print(f"[P-A60.5] patient_id set 일치: {pid_match} (누락={len(missing_csv)}, 초과={len(extra_csv)})")

    # group 확인
    group_counts = dict(Counter(pid2group.get(pid, "UNKNOWN") for pid in csv_pids))
    print(f"[P-A60.5] group_counts: {group_counts}")

    # reference row count (P-A20 ResNet50 rand224)
        ...
        for fp in REF_SCORE_DIR.glob("*.csv"):
            ...
            if pid in stage1_dev_ids:
                try:
                    ...
                    pass
        print(f"[P-A60.5] reference (P-A20) row count 로드: {len(ref_row_counts)}명")
    else:
        print(f"[P-A60.5] reference (P-A20) score dir 없음 → 내부 정합만 확인")

    # 컬럼 체크용 (첫 번째 CSV에서 확인)
    ...
    has_local_z = "local_z" in actual_cols or "z_level" in actual_cols
    has_lesion_label = "has_lesion_patch" in actual_cols or "patch_label" in actual_cols
    col_check_ok = len(required_missing) == 0
    print(f"[P-A60.5] 컬럼 체크: required_present={len(required_present)}, missing={required_missing}")
    ...
    # 통계 계산
    if n_finite_total:
        mean = s_sum / n_finite_total
        var = max(s_sumsq / n_finite_total - mean ** 2, 0.0)
        std = math.sqrt(var)
    else:
        mean = std = float("nan")
    ...
    # P-A60 보고서 값과 비교
    patch_diff = n_total_robust - P_A60_REPORTED_PATCHES
    p95_exceed_diff = n_over_p95 - P_A60_P95_EXCEED
    p99_exceed_diff = n_over_p99 - P_A60_P99_EXCEED

    # np.loadtxt quirk 여부
    loadtxt_quirk_detected = (patch_diff == 1)
    loadtxt_quirk_note = (
        f"patch_diff={patch_diff}: robust={n_total_robust} vs p_a60_reported={P_A60_REPORTED_PATCHES}. "
        + ("np.loadtxt quirk 재현 (MSD_lung_071 1행 누락 패턴)" if loadtxt_quirk_detected else
           "차이 없음 또는 다른 원인")
    )

    # label/mask 연결 가능 여부 확인
    label_connectable = "has_lesion_patch" in actual_cols or "patch_label" in actual_cols
    mask_connectable = "lesion_pixels" in actual_cols  # lesion_pixels>0 이면 lesion mask 연결 가능
    p61_ready = (
        col_check_ok
        and n_nan_total == 0
        and n_inf_total == 0
    ...
    verdict = "통과" if p61_ready else ("부분통과" if n_total_robust > 0 else "실패")
    ...
    with open(SUMMARY_CSV, "w", encoding="utf-8-sig", newline="") as f:
        ...
        for k, v in [
            ...
            ("p_a60_reported_patches", P_A60_REPORTED_PATCHES),
            ("patch_count_diff", patch_diff),
            ("loadtxt_quirk_detected", loadtxt_quirk_detected),
            ("n_nan_total", n_nan_total),
            ("n_inf_total", n_inf_total),
            ("score_min_robust", s_min),
            ("score_max_robust", s_max),
            ("score_mean_robust", mean),
            ("score_std_robust", std),
            ("threshold_p95", p95),
            ("threshold_p99", p99),
            ("n_over_p95_robust", n_over_p95),
            ("ratio_over_p95_robust", ratio_p95),
            ("p_a60_p95_exceed", P_A60_P95_EXCEED),
            ("p95_exceed_diff", p95_exceed_diff),
            ("n_over_p99_robust", n_over_p99),
            ("ratio_over_p99_robust", ratio_p99),
            ("p_a60_p99_exceed", P_A60_P99_EXCEED),
            ("p99_exceed_diff", p99_exceed_diff),
    ...
    _write_md(REPORT_MD, {
        ...
        "p60_reported": P_A60_REPORTED_PATCHES,
        ...
        "p60_p95": P_A60_P95_EXCEED, "p95_diff": p95_exceed_diff,
        "n_over_p99": n_over_p99, "ratio_p99": ratio_p99,
        "p60_p99": P_A60_P99_EXCEED, "p99_diff": p99_exceed_diff,
    ...
    # JSON
    json_result = {
        "stage": "P-A60.5_score_artifact_validation_resnet18_rand224",
        ...
        "p_a60_p95_exceed": P_A60_P95_EXCEED, "p95_exceed_diff": p95_exceed_diff,
        "n_over_p99": n_over_p99, "ratio_over_p99": ratio_p99,
        "p_a60_p99_exceed": P_A60_P99_EXCEED, "p99_exceed_diff": p99_exceed_diff,
        "required_cols_all_present": col_check_ok,
    ...
    print(f"\n[P-A60.5] 완료: {elapsed:.1f}s")
    print(f"[P-A60.5] robust_total_patches={n_total_robust:,} (P-A60_reported={P_A60_REPORTED_PATCHES:,}, diff={patch_diff:+d})")
    print(f"[P-A60.5] loadtxt quirk detected: {loadtxt_quirk_detected}")
    print(f"[P-A60.5] nan={n_nan_total}, inf={n_inf_total}")
    if not math.isnan(mean):
        print(f"[P-A60.5] score min/max/mean/std: {s_min:.4f}/{s_max:.4f}/{mean:.4f}/{std:.4f}")
    print(f"[P-A60.5] p95 초과: {n_over_p95:,} (diff={p95_exceed_diff:+d}), p99 초과: {n_over_p99:,} (diff={p99_exceed_diff:+d})")
    print(f"[P-A60.5] p61_ready: {p61_ready}")
    print(f"[P-A60.5] 판정: {verdict}")
    print(f"[P-A60.5] 보고서: {REPORT_MD}")


def _write_md(path: Path, s: dict) -> None:
    L = ["# P-A60.5 score artifact validation 보고서 (ResNet18 rand224)\n",
         f"## 판정: {s['verdict']}\n",
         ...
         f"- P-A60 reported patches: {s['p60_reported']:,}",
         f"- 차이: {s['patch_diff']:+d}",
         ...
         f"- score min (robust): {s['score_min']:.6f}" if not math.isnan(s['score_min']) else "- score min: -",
         f"- score max (robust): {s['score_max']:.6f}" if not math.isnan(s['score_max']) else "- score max: -",
         f"- score mean (robust): {s['score_mean']:.6f}" if not math.isnan(s['score_mean']) else "- score mean: -",
         f"- score std (robust): {s['score_std']:.6f}" if not math.isnan(s['score_std']) else "- score std: -",
         f"- P-A60 보고서 mean/std: 18.7626 / 5.1183\n",
         "## p95/p99 exceedance 정합",
         f"- 사용 threshold p95={s['p95']:.6f}, p99={s['p99']:.6f}",
         f"- p95 초과 (robust): {s['n_over_p95']:,} ({s['ratio_p95']:.4%}) vs P-A60={s['p60_p95']:,} (diff={s['p95_diff']:+d})",
         f"- p99 초과 (robust): {s['n_over_p99']:,} ({s['ratio_p99']:.4%}) vs P-A60={s['p60_p99']:,} (diff={s['p99_diff']:+d})\n",
         "## 컬럼 검증",
         ...
         "## P-A61 metrics 진행 가능 여부",
         f"- **{s['p61_ready']}** — 사용자 승인 후 P-A61 진행 가능\n",
         "## 검증 체크",
         f"- scoring/forward/training 미실행: ✅",
         f"- metrics 미계산: ✅",
         f"- stage2_holdout 접근 0: ✅",
         f"- 기존 결과 무수정: ✅",
         f"- reference (P-A20) row count 비교: {'가능' if s['ref_available'] else '참조 파일 없음'}",
