"""
P-A55: ResNet18 random224 train smoke limit=5

normal train split 중 5명만 사용해 PaDiM 분포 학습 파이프라인 검증.
- FeatureExtractor (ResNet18 ImageNet1K V1)
- PaDiMModel (feature_dim=224, selected_feature_indices rand224)
- smoke output: outputs/smoke/train_limit5/position_bin_stats.npz

금지: full train / limit>5 / scoring / threshold / metrics / stage2_holdout
"""
...
LIMIT = 5
SCRIPT_NAME = "p_a55_train_smoke_limit5.py"
SMOKE_OUT = ws_paths.resolve_train_output(full_run=False, limit=LIMIT)
OUTPUT_NPZ = SMOKE_OUT["model_npz"]
REPORTS_DIR = SMOKE_OUT["reports_dir"]
ERROR_CSV = REPORTS_DIR / "error.csv"
RUNTIME_CSV = REPORTS_DIR / "p_a55_runtime_summary.csv"
...
def main():
    ...
    with open(p54_json) as f:
        p54 = json.load(f)
    assert p54.get("verdict") == "통과", f"P-A54 판정 미통과: {p54.get('verdict')}"
    print("[guard1] P-A54 보고서 통과 확인 OK")

    # ---- 가드 2~5: selected_feature_indices 검증 ----
    ...
    assert sidx.min() >= 0 and sidx.max() <= 447, f"[guard4] range 오류: {sidx.min()}~{sidx.max()}"
    ...
    assert set(existing_idx.tolist()).issubset(set(sidx.tolist())), "[guard5] 기존 100개 미포함"
    print(f"[guard2-5] selected_feature_indices OK: shape={sidx.shape}, unique=224, range=[{sidx.min()},{sidx.max()}], 기존100포함=True")
    ...
    if OUTPUT_NPZ.exists():
        print(f"[guard7][ABORT] smoke output이 이미 존재합니다: {OUTPUT_NPZ}")
        print("  기존 결과를 수동으로 삭제하거나 경로를 확인하세요.")
        sys.exit(1)
    print(f"[guard7] smoke output 경로 비어있음 OK: {OUTPUT_NPZ}")

    # ---- 가드 8: full train 경로와 smoke 경로 분리 확인 ----
    assert OUTPUT_NPZ != ws_paths.MODEL_NPZ, "[guard8] smoke/full 경로 충돌"
    print(f"[guard8] smoke/full 경로 분리 OK")

    # ---- 가드 9: stage2_holdout 접근 금지 ----
    print(f"[guard9] stage2_holdout 접근 금지 확인 OK (접근 없음)")

    # ---- 가드 10: P-A54 limit1 결과 보존 확인 ----
    p54_npz = ws_paths.SMOKE_ROOT / "train_limit1" / "position_bin_stats.npz"
    assert p54_npz.exists(), f"[guard10] P-A54 limit1 결과가 없습니다: {p54_npz}"
    print(f"[guard10] P-A54 limit1 결과 보존 확인 OK: {p54_npz}")
    ...
    print(f"[train] smoke output: {OUTPUT_NPZ}")
    ...
    report = {
        ...
        "p_a54_limit1_preserved": str(p54_npz),
        "next_step_p_a56_full_train_preflight_feasible": True,
    }
    report_json_path = REPORTS_DIR / "p_a55_train_smoke_limit5.json"
    ...
    # ---- 보고서 MD ----
    md_lines = [
        "# P-A55 Train Smoke limit=5 보고서",
        "",
        f"## 판정: 통과",
        "",
        "## 실행 정보",
        f"- 처리 환자 수: {summary['n_patients_success']}명 (요청 {LIMIT}명)",
        ...
        f"- 총 patch 수 (used): {summary['n_patches_used']:,}",
        ...
        f"- P-A54 limit1 결과 보존: {p54_npz}",
        f"- 기존 random100 결과 무수정: True (접근 없음)",
        f"- full train 미실행: True",
        f"- scoring/threshold/metrics 미실행: True",
        f"- lesion/stage2_holdout 미접근: True",
        "",
        "## 다음 단계",
        f"- P-A56 full train preflight 가능 여부: True",
    ]
    report_md_path = REPORTS_DIR / "p_a55_train_smoke_limit5.md"
    with open(report_md_path, "w", encoding="utf-8") as f:
        f.write("\n".join(md_lines))
    print(f"[train] 보고서 MD: {report_md_path}")
    print("\n=== P-A55 smoke limit5 완료: 통과 ===")
