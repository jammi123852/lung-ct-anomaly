"""
P-A54: ResNet18 random224 train smoke limit=1

normal train split 중 1명만 사용해 PaDiM 분포 학습 파이프라인 검증.
- FeatureExtractor (ResNet18 ImageNet1K V1)
- PaDiMModel (feature_dim=224, selected_feature_indices rand224)
- smoke output: outputs/smoke/train_limit1/position_bin_stats.npz

금지: full train / limit>1 / scoring / threshold / metrics / stage2_holdout
"""
...
LIMIT = 1
SCRIPT_NAME = "p_a54_train_smoke_limit1.py"
...
def main():
    ...
    # ---- 가드 1: P-A53 보고서 통과 확인 ----
    p53_json = (ws_paths.EXP_ROOT / "outputs" / "reports" / "p_a53_selected_indices.json")
    assert p53_json.exists(), f"P-A53 보고서 없음: {p53_json}"
    with open(p53_json) as f:
        p53 = json.load(f)
    assert p53.get("verdict") == "통과", f"P-A53 판정 미통과: {p53.get('verdict')}"
    print("[guard1] P-A53 보고서 통과 확인 OK")
    ...
    # ---- 가드 9: stage2_holdout 접근 금지 (경로 사전 차단) ----
    stage2_holdout_marker = REPO_ROOT / "data" / "stage2_holdout"
    ...
    report = {
        ...
        "next_step_p_a55_train_smoke_limit5_feasible": True,
    }
    report_json_path = REPORTS_DIR / "p_a54_train_smoke_limit1.json"
    with open(report_json_path, "w") as f:
        json.dump(report, f, ensure_ascii=False, indent=2)
    print(f"[train] 보고서 JSON: {report_json_path}")
    print("\n=== P-A54 smoke limit1 완료: 통과 ===")
