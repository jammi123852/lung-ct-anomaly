#!/usr/bin/env python3
"""
build_explanation_card_s4_reason_card_prototype_1case_v7_textfix.py

S4 Reason Card Prototype 1-case v7_textfix — reason text 보수화 (혈관성→고밀도)

목적:
- v7 눈검증 PARTIAL_PASS 후 text/reference policy tuning preflight PASS를 반영.
- v7 대비 변경 사항:
    1. REASON_TEXT_KO: "연부조직/혈관성 구조" → "밝은 고밀도 구조" (보수화)
       "정상 예시들보다" 추가로 reference 비교 의미 강화
       disclaimer 문장 통합
    2. REASON_TEXT_EN: "soft-tissue or vessel-like structure" → "bright high-density structures"
       "normal examples" 비교 의미 추가
    3. output root: v7_textfix (v7 보존)
    4. reference policy note 및 z-context 생략 정책 상수 추가

v7과 동일하게 유지하는 항목:
  - layout (Row1/Row2/Row3 구조)
  - crop 설계 (candidate, matched normal, addref1, addref2)
  - display size (320px / 180px)
  - HU window (center=-600, width=1500)
  - reference selection (same-bin lower_peripheral, v7 reference set 그대로)
  - 한글 font fix (Malgun Gothic)
  - guard 기본값 (ALLOW_RUN_CARD_PROTOTYPE=False 등)
...
syntax check:
  python -m py_compile scripts/build_explanation_card_s4_reason_card_prototype_1case_v7_textfix.py
"""
...
REASON_TEXT_KO = (
    ...
    "후보 crop은 정상 예시들보다 공기 음영이 적고 밝은 고밀도 구조가 더 넓게 보였습니다. "
    "HU 밀도도 약 +245 HU 높게 측정되었으며, "
    "이는 PaDiM high-response를 해석하기 위한 시각적 참고 단서일 뿐 진단 의미는 아닙니다."
)

REASON_TEXT_EN = (
    "Compared with same-bin normal CT crops in the lower_peripheral region "
    "using the same crop size and window, the candidate crop showed less "
    "air-like appearance and broader bright high-density structures than the "
    "normal examples. Its HU density was also about +245 HU higher; "
...
# v7_textfix 변경 기록
# ============================================================
TEXTFIX_VERSION = "v7_textfix"
TEXTFIX_CHANGES = {
    "ko_removed": "연부조직/혈관성 구조",
    "ko_added": "밝은 고밀도 구조, 정상 예시들보다",
    "en_removed": "soft-tissue or vessel-like structure",
    "en_added": "bright high-density structures, normal examples",
    "reason": "v7 눈검증 PARTIAL_PASS — 외부 공유용 보수적 표현으로 완화",
    "based_on_preflight": (
        "reports/explanation_cards/"
        "s4_reason_card_prototype_v7_text_reference_policy_tuning_preflight_v1.md"
    ),
}

# ============================================================
# reference selection policy note (v7_textfix — full 전 미구현 항목 기록)
# ============================================================
REFERENCE_POLICY_NOTE = {
    "current_policy": "same position_bin lower_peripheral, v7 reference set 그대로",
    "mandatory_conditions_met": [
        "same position_bin 일치",
        "원본 CT path + slice + bbox resolve 가능",
        "crop size 96x96 유지 가능",
        "stage2_holdout 제외",
        "빈/경계 밖 crop 제외",
    ],
    "recommended_conditions_pending_full_run": [
        "peripheral/pleural proximity 기반 reference 우선 (조건 6)",
        "HU distribution이 reference pool 중앙값에 가까운 normal 우선 (조건 7)",
        "diversity quality filter 강화 (조건 8)",
    ],
    "note": (
        "v7_textfix는 v7 reference set을 그대로 사용한다. "
        "조건 6~8은 full 300 적용 전까지 구현 목표."
    ),
}

# ============================================================
# D z-context 생략 정책 (comparison-first prototype)
# ============================================================
Z_CONTEXT_OMISSION_POLICY = (
    "v7_textfix reason card는 PaDiM high-response의 시각적 비교 근거를 보여주기 위한 "
    "comparison-first prototype이다. z-context는 기존 S3 card에서 확인 가능하므로, "
    "본 prototype에서는 candidate-vs-normal reference 비교와 reason text를 우선한다."
)
...
PROTO_V7_OUTPUT_ROOT = CARDS_VIS_ROOT / "s4_reason_card_prototype_1case_v7_highres_multi_ref"

# ── v7_textfix 출력 경로 ─────────────────────────────────────
PROTO_OUTPUT_ROOT    = CARDS_VIS_ROOT / "s4_reason_card_prototype_1case_v7_textfix"
...
def validate_reason_text() -> Dict[str, Any]:
    """reason text v7_textfix 검사 (v7 검사 + textfix 추가 항목)."""
    issues = []
    ...
    # ── textfix 추가 검사 ──────────────────────────────────────
    has_highdens = "밝은 고밀도 구조" in REASON_TEXT_KO
    if not has_highdens:
        issues.append("textfix: KO text에 '밝은 고밀도 구조' 없음")

    has_vessel_ko = "혈관성 구조" in REASON_TEXT_KO
    if has_vessel_ko:
        issues.append("textfix: KO text에 '혈관성 구조' 잔존 — 제거 필요")

    has_vessel_en = "vessel-like" in REASON_TEXT_EN
    if has_vessel_en:
        issues.append("textfix: EN text에 'vessel-like' 잔존 — 제거 필요")

    has_not_diagnosis = "not a diagnosis" in REASON_TEXT_EN
    if not has_not_diagnosis:
        issues.append("textfix: EN text에 'not a diagnosis' 없음")

    has_245hu_en = "about +245 HU" in REASON_TEXT_EN
    if not has_245hu_en:
        issues.append("textfix: EN text에 'about +245 HU' 없음")
    ...
    return {
        ...
        # textfix 추가
        "textfix_has_highdens_ko": has_highdens,
        "textfix_no_vessel_ko": not has_vessel_ko,
        "textfix_no_vessel_en": not has_vessel_en,
        "textfix_has_not_diagnosis_en": has_not_diagnosis,
        "textfix_has_245hu_en": has_245hu_en,
    }
...
def run_selftest() -> bool:
    ...
    print("=== selftest (v7_textfix — reason text 보수화) ===")
    ...
    # 추가 항목
    _check("K1. output root v1~v7와 분리 (v7_textfix)",
           PROTO_OUTPUT_ROOT.name == "s4_reason_card_prototype_1case_v7_textfix",
    ...
    _check("K9. v1~v7 prototype 보존 확인",
           all([
    ...
    # ── textfix 전용 검사 (T 항목) ────────────────────────────
    text_res2 = validate_reason_text()

    _check("T1. KO text '밝은 고밀도 구조' 포함",
           text_res2.get("textfix_has_highdens_ko", False),
           "포함됨" if text_res2.get("textfix_has_highdens_ko") else "없음 — textfix 실패")

    _check("T2. KO text '혈관성 구조' 없음",
           text_res2.get("textfix_no_vessel_ko", False),
           "제거됨" if text_res2.get("textfix_no_vessel_ko") else "잔존 — textfix 실패")

    _check("T3. EN text 'vessel-like' 없음",
           text_res2.get("textfix_no_vessel_en", False),
           "제거됨" if text_res2.get("textfix_no_vessel_en") else "잔존 — textfix 실패")

    _check("T4. EN text 'not a diagnosis' 포함",
           text_res2.get("textfix_has_not_diagnosis_en", False),
           "포함됨" if text_res2.get("textfix_has_not_diagnosis_en") else "없음")

    _check("T5. EN text 'about +245 HU' 포함",
           text_res2.get("textfix_has_245hu_en", False),
           "포함됨" if text_res2.get("textfix_has_245hu_en") else "없음")

    _check("T6. v7 output root 보존 목록에 있음",
           PROTO_V7_OUTPUT_ROOT is not None
           and PROTO_V7_OUTPUT_ROOT.name == "s4_reason_card_prototype_1case_v7_highres_multi_ref",
           PROTO_V7_OUTPUT_ROOT.name if PROTO_V7_OUTPUT_ROOT else "없음")

    _check("T7. PROTOTYPE_VERSION=v7_textfix",
           PROTOTYPE_VERSION == "v7_textfix",
           f"value={PROTOTYPE_VERSION}")

    _check("T8. REFERENCE_POLICY_NOTE 상수 존재",
           isinstance(REFERENCE_POLICY_NOTE, dict) and "mandatory_conditions_met" in REFERENCE_POLICY_NOTE,
           "존재" if isinstance(REFERENCE_POLICY_NOTE, dict) else "없음")

    _check("T9. Z_CONTEXT_OMISSION_POLICY 상수 존재",
           isinstance(Z_CONTEXT_OMISSION_POLICY, str) and "comparison-first" in Z_CONTEXT_OMISSION_POLICY,
           "존재" if isinstance(Z_CONTEXT_OMISSION_POLICY, str) else "없음")

    _check("T10. output root name=v7_textfix",
           PROTO_OUTPUT_ROOT.name == "s4_reason_card_prototype_1case_v7_textfix",
           PROTO_OUTPUT_ROOT.name)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="S4 Reason Card Prototype 1-case v7_textfix — reason text 보수화 (혈관성→고밀도)",
