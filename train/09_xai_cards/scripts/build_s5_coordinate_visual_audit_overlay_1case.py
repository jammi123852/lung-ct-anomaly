"""
S5 Coordinate Visual Audit Overlay — 1-case
=============================================
대상: LUNG1-320__c2
목적: patch_id=4(center)와 patch_id=5(max/right-adjacent)를
      CT local_z=89 slice 위에 overlay하는 시각 검증용 PNG 생성

실행 방법:
  --selftest                              : 정적 selftest 40+ 항목 (허용)
...
WARNING-1 (S4/S5 slice 불일치):
  ...
  Overlay v1은 S5 local_z=89 기준으로만 해석.
  S4 PNG와 pixel 직접 비교 금지.

WARNING-2 (HU window 불일치):
  S4 visual card: lung window center=-600, width=1500
  S5 preprocessing: HU_MIN=-1000, HU_MAX=200
  Overlay v1은 lung window(center=-600, width=1500) 우선.
  PaDiM preprocessing window overlay는 v2 후보로만 기록.
...
CT_INDEX_Z         = 89     # CT npy[89] — S5 contribution map 기준
REPORT_SLICE_INDEX = 140    # reporting/title 표기 전용 (CT index 아님)
...
# ============================================================
# B. HU window constants
# ============================================================

# v1 overlay: lung window (S4 visual card 기준)
WINDOW_MODE          = "lung"
LUNG_WINDOW_CENTER   = -600
LUNG_WINDOW_WIDTH    = 1500
LUNG_HU_LOW          = LUNG_WINDOW_CENTER - LUNG_WINDOW_WIDTH // 2   # -1350
LUNG_HU_HIGH         = LUNG_WINDOW_CENTER + LUNG_WINDOW_WIDTH // 2   #  150

# S5 preprocessing 기록 (v2 후보용)
S5_PREPROCESS_HU_MIN = -1000
S5_PREPROCESS_HU_MAX =  200

# Mismatch warning string (selftest에서 확인)
_HU_WINDOW_MISMATCH_WARNING = (
    "S4 lung window(center=-600, width=1500)와 S5 preprocessing window"
    "(HU_MIN=-1000, HU_MAX=200)가 다름. Overlay v1은 lung window 우선."
)
_S4_S5_SLICE_MISMATCH_WARNING = (
    "S4 v7_textfix는 CANDIDATE_SLICE_Z=140을 CT index로 사용. "
    "S5 contribution map은 local_z=89 사용. 서로 다른 slice. "
    "Overlay v1 기준: local_z=89. S4 PNG와 pixel 직접 비교 금지."
)
...
PREFLIGHT_JSON = os.path.join(
    REPO_ROOT,
    "reports/explanation_cards/s5_coordinate_visual_audit_preflight_v1.json"
)
PATCH_SCORE_REVIEW_CSV = os.path.join(
    REPO_ROOT,
    "reports/explanation_cards/s5_patch_level_contribution_map_patch_score_review_v1.csv"
)
...
OUTPUT_PNG_PATCH4_PATCH5 = os.path.join(OUTPUT_ROOT, "coordinate_overlay_patch4_patch5.png")
OUTPUT_PNG_3X3_GRID      = os.path.join(OUTPUT_ROOT, "coordinate_overlay_3x3_grid.png")
...
def run_selftest() -> int:
    ...
    # --- 2. CASE_ID / VOLUME_ID ---
    chk("07_case_id_exact",   CASE_ID == "LUNG1-320__c2",
        f"got {CASE_ID}")
    chk("08_volume_id_exact", VOLUME_ID == "NSCLC_LUNG1-320__95de24d86f",
        f"got {VOLUME_ID}")

    # --- 3. local_z / slice_index ---
    chk("09_ct_index_z_89",              CT_INDEX_Z == 89,
        f"got {CT_INDEX_Z}")
    chk("10_report_slice_index_140",     REPORT_SLICE_INDEX == 140,
        f"got {REPORT_SLICE_INDEX}")
    chk("11_ct_index_z_ne_slice_index",  CT_INDEX_Z != REPORT_SLICE_INDEX,
        f"local_z({CT_INDEX_Z}) == slice_index({REPORT_SLICE_INDEX}) — 혼동 위험")
    chk("12_local_z_flag_true",          LOCAL_Z_USED_FOR_CT_INDEXING is True,
        "LOCAL_Z_USED_FOR_CT_INDEXING must be True")
    chk("13_slice_index_flag_false",     SLICE_INDEX_USED_FOR_CT_INDEXING is False,
        "SLICE_INDEX_USED_FOR_CT_INDEXING must be False")

    # --- 4. patch4 bbox ---
    chk("14_patch4_bbox_exact",          PATCH4_BBOX == [320, 208, 352, 240],
        f"got {PATCH4_BBOX}")
    chk("15_patch4_width_32",            PATCH4_BBOX[3] - PATCH4_BBOX[1] == 32,
        f"patch4 width={PATCH4_BBOX[3]-PATCH4_BBOX[1]}")
    chk("16_patch4_height_32",           PATCH4_BBOX[2] - PATCH4_BBOX[0] == 32,
        f"patch4 height={PATCH4_BBOX[2]-PATCH4_BBOX[0]}")

    # --- 5. patch5 bbox ---
    chk("17_patch5_bbox_exact",          PATCH5_BBOX == [320, 240, 352, 272],
        f"got {PATCH5_BBOX}")
    chk("18_patch5_width_32",            PATCH5_BBOX[3] - PATCH5_BBOX[1] == 32,
        f"patch5 width={PATCH5_BBOX[3]-PATCH5_BBOX[1]}")
    chk("19_patch5_height_32",           PATCH5_BBOX[2] - PATCH5_BBOX[0] == 32,
        f"patch5 height={PATCH5_BBOX[2]-PATCH5_BBOX[0]}")
    ...
    # --- 7. grid extent ---
    chk("22_grid_extent_exact",          GRID_EXTENT == [288, 176, 384, 272],
        f"got {GRID_EXTENT}")
    gy0, gx0, gy1, gx1 = GRID_EXTENT
    ...
    chk("24_all_bboxes_in_512",          len(bbox_errors) == 0,
        f"bbox errors: {bbox_errors}")

    # --- 9. lung window ---
    chk("25_lung_window_center_neg600",  LUNG_WINDOW_CENTER == -600,
        f"got {LUNG_WINDOW_CENTER}")
    chk("26_lung_window_width_1500",     LUNG_WINDOW_WIDTH == 1500,
        f"got {LUNG_WINDOW_WIDTH}")
    chk("27_lung_hu_low_neg1350",        LUNG_HU_LOW == -1350,
        f"got {LUNG_HU_LOW}")
    chk("28_lung_hu_high_150",           LUNG_HU_HIGH == 150,
        f"got {LUNG_HU_HIGH}")

    # --- 10. S5 preprocessing window recorded ---
    chk("29_s5_hu_min_neg1000",          S5_PREPROCESS_HU_MIN == -1000,
        f"got {S5_PREPROCESS_HU_MIN}")
    chk("30_s5_hu_max_200",              S5_PREPROCESS_HU_MAX == 200,
        f"got {S5_PREPROCESS_HU_MAX}")

    # --- 11. S4/S5 slice mismatch warning ---
    chk("31_slice_mismatch_warning_exists",
        len(_S4_S5_SLICE_MISMATCH_WARNING) > 0,
        "S4/S5 slice mismatch warning string empty")
    chk("32_slice_mismatch_warning_has_140",
        "140" in _S4_S5_SLICE_MISMATCH_WARNING,
        "slice mismatch warning must mention 140")
    chk("33_slice_mismatch_warning_has_89",
        "89" in _S4_S5_SLICE_MISMATCH_WARNING,
        "slice mismatch warning must mention 89")
    chk("34_s4_pixel_comparison_forbidden",
        "직접 비교 금지" in _S4_S5_SLICE_MISMATCH_WARNING,
        "S4 pixel direct comparison prohibition missing")

    # --- 12. HU window mismatch warning ---
    chk("35_hu_window_mismatch_warning_exists",
        len(_HU_WINDOW_MISMATCH_WARNING) > 0,
        "HU window mismatch warning string empty")
    chk("36_hu_window_mismatch_lung_mentioned",
        "lung window" in _HU_WINDOW_MISMATCH_WARNING.lower() or
        "center=-600" in _HU_WINDOW_MISMATCH_WARNING,
        "HU window mismatch must mention lung window")

    # --- 13. mmap_mode policy ---
    chk("37_mmap_mode_policy_exists",    "mmap_mode" in _MMAP_MODE_POLICY,
        "mmap_mode policy string missing")
    chk("38_mmap_mode_r_required",       "mmap_mode='r'" in _MMAP_MODE_POLICY,
        "mmap_mode='r' not stated in policy")

    # --- 14. dry-run policy strings ---
    chk("39_dryrun_no_ct_load",          "CT load 없음" in _DRY_RUN_POLICY,
        "dry-run must state no CT load")
    chk("40_dryrun_no_overlay_render",   "overlay render 없음" in _DRY_RUN_POLICY,
        "dry-run must state no overlay render")
    chk("41_dryrun_no_png_write",        "PNG write 없음" in _DRY_RUN_POLICY,
        "dry-run must state no PNG write")
    chk("42_dryrun_no_model_forward",    "model forward 없음" in _DRY_RUN_POLICY,
        "dry-run must state no model forward")
    chk("43_dryrun_no_feature_extract",  "feature extraction 없음" in _DRY_RUN_POLICY,
        "dry-run must state no feature extraction")
    chk("44_dryrun_no_contribution_recalc",
        "contribution 재계산 없음" in _DRY_RUN_POLICY,
        "dry-run must state no contribution recalc")

    # --- 15. naming/caveat policy ---
    chk("45_not_gradcam_caveat",         "Not Grad-CAM" in OVERLAY_CAVEAT,
        "caveat must state Not Grad-CAM")
    chk("46_not_pixel_attribution_caveat",
        "not pixel attribution" in OVERLAY_CAVEAT,
        "caveat must state not pixel attribution")
    chk("47_not_diagnostic_caveat",      "not diagnostic" in OVERLAY_CAVEAT,
        "caveat must state not diagnostic")

    # --- 16. output root ---
    chk("48_output_root_in_outputs",
        "outputs/" in OUTPUT_ROOT,
        f"output root outside outputs/: {OUTPUT_ROOT}")
    chk("49_output_root_s5_coordinate",
        "s5_coordinate_visual_audit" in OUTPUT_ROOT,
        f"output root must be s5_coordinate_visual_audit specific: {OUTPUT_ROOT}")

    # --- 17. stage2_holdout not in CT path ---
    chk("50_stage2_not_in_ct_path",
        _STAGE2_HOLDOUT_SENTINEL not in CT_HU_NPY_PATH,
        "CT 경로에 stage2_holdout 포함 위험")

    # --- 18. run mode requires both flags (inspect run_overlay source) ---
    import inspect
    src = inspect.getsource(run_overlay)
    chk("51_run_requires_run_overlay_flag",
        "run_overlay" in src,
        "run_overlay에 --run-overlay 체크 없음")
    chk("52_run_requires_confirm_overlay_flag",
        "confirm_overlay" in src,
        "run_overlay에 --confirm-overlay 체크 없음")
    chk("53_run_checks_all_guards",
        "ALLOW_CT_LOAD" in src and "ALLOW_OVERLAY_RENDER" in src and "ALLOW_PNG_WRITE" in src,
        "run_overlay must check all 3 run guards")

    # --- 19. image dimensions ---
    chk("54_image_h_512",    IMAGE_H == 512, f"got {IMAGE_H}")
    chk("55_image_w_512",    IMAGE_W == 512, f"got {IMAGE_W}")
...
def run_dry_run() -> int:
    ...
    paths_to_check = [
        ...
        ("PATCH_SCORE_REVIEW_CSV",  PATCH_SCORE_REVIEW_CSV),
    ]
    missing = []
    ...
    if os.path.exists(OUTPUT_ROOT):
        print(f"[INFO] output root 이미 존재: {OUTPUT_ROOT}")
    else:
...
def run_plan_only() -> int:
    ...
    print(f"  v1 (overlay v1 기준): lung window center={LUNG_WINDOW_CENTER}, width={LUNG_WINDOW_WIDTH}")
    print(f"    HU range: [{LUNG_HU_LOW}, {LUNG_HU_HIGH}]")
    print(f"    norm = clip((hu - {LUNG_HU_LOW}) / {LUNG_WINDOW_WIDTH}, 0, 1)")
    print(f"  v2 후보: PaDiM preprocessing window HU_MIN={S5_PREPROCESS_HU_MIN}, HU_MAX={S5_PREPROCESS_HU_MAX}")
    print(f"  [WARNING] {_HU_WINDOW_MISMATCH_WARNING}")
    print()
    ...
    print(f"  [NOTE] 3×3 grid extent = S4 candidate crop 96×96 범위와 정확히 일치")
    print()
    ...
    outputs = [
        ("coordinate_overlay_patch4_patch5.png",  "lung window, patch4(yellow)+patch5(red) only"),
        ("coordinate_overlay_3x3_grid.png",       "lung window, patch4(yellow)+patch5(red)+3×3 grid(gray)"),
        ("coordinate_overlay_metadata.json",      "파라미터 기록"),
...
def run_overlay(args: argparse.Namespace) -> int:
    ...
    # --- lung window windowing ---
    hu_low  = LUNG_HU_LOW    # -1350
    hu_high = LUNG_HU_HIGH   #  150
    ct_windowed = np.clip(ct_slice_raw, hu_low, hu_high)
    ct_display  = ((ct_windowed - hu_low) / (hu_high - hu_low) * 255).astype(np.uint8)

    # ============================================================
    # PNG 1: patch4(yellow) + patch5(red thick) — patch4_patch5
    # ============================================================
    fig1, ax1 = plt.subplots(1, 1, figsize=(7, 7))
    ax1.imshow(ct_display, cmap="gray", vmin=0, vmax=255, origin="upper")

    # patch4: [y0,x0,y1,x1]=[320,208,352,240] → matplotlib rect: (x0, y0, width, height)
    ...
    # patch5: [y0,x0,y1,x1]=[320,240,352,272]
    ...
    # ============================================================
    # PNG 2: 3×3 grid extent + patch4/patch5 강조
    # ============================================================
    fig2, ax2 = plt.subplots(1, 1, figsize=(7, 7))
    ax2.imshow(ct_display, cmap="gray", vmin=0, vmax=255, origin="upper")

    # full 3×3 grid: thin neutral boxes
    for p in PATCH_GRID:
        pid = p["patch_id"]
    ...
    # patch5 red thick overlay on grid
    ...
    metadata = {
        ...
        "hu_window_mismatch_warning": _HU_WINDOW_MISMATCH_WARNING,
    ...
    index_rows = [
        {"file": os.path.basename(OUTPUT_PNG_PATCH4_PATCH5), "type": "overlay_png",
         "description": "patch4_yellow + patch5_red_thick on CT local_z=89"},
        {"file": os.path.basename(OUTPUT_PNG_3X3_GRID), "type": "overlay_png",
         "description": "full 3x3 grid + patch4 + patch5 on CT local_z=89"},
        {"file": os.path.basename(OUTPUT_METADATA_JSON), "type": "metadata_json",
         "description": "overlay generation metadata and policy records"},
    ]
    ...
    with open(os.path.join(OUTPUT_ROOT, "runtime_summary.json"), "w", encoding="utf-8") as f:
        json.dump(runtime_summary, f, ensure_ascii=False, indent=2)
    generated.append(os.path.join(OUTPUT_ROOT, "runtime_summary.json"))
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="S5 Coordinate Visual Audit Overlay — 1-case"
    )
