...
# -*- coding: utf-8 -*-
"""
Explanation Card S3 v2 : prototype explanation card 생성기 (local display bbox + overmerge flag)

기준:
  - reports/explanation_cards/s3_overmerge_fix_design_v1.md (안 D: B2 우선)
  - reports/explanation_cards/s3_card_v2_script_fix_preflight_v1.md (PASS)
  - v1 스크립트 build_explanation_card_s3_prototype_cards.py (참고/보존)

v2 핵심 변경:
  - 카드 표시 bbox = display_bbox = max-score patch bbox + margin 32 (component union bbox 폐기, JSON엔 둘 다 보존).
    over-merge component에서 display area 평균 ~79% 축소(localization 회복).
  - overmerge_flag(large_union/extreme_union) + "broad component response / 광역 component 반응" 병기(진단 표현 금지).
  - JSON 숫자 필드 int/float/bool 명확 캐스팅. diagnostic_terms_blocked는 list 유지 + bool diagnostic_guard_passed 추가.
  - 신규 출력 s3_prototype_cards_v2/ (v1/S2/S1 무수정).

max-score patch 식별:
  - patch_candidates.csv 를 prototype safe_id 만 streaming. (safe_id, position_bin, max_score_slice_index,
    padim_score≈max) 매칭. 동점은 component union bbox 내부/근접 우선. preflight 24/24 직접식별 확인.
...
def _generate_cards(out_dir):
    ...
    for row in rows:
        ...
        try:
            ...
            axA.set_title("A. slice z=%d  %s  rank=%s%s" % (
                zc, row["position_bin"], row["rank_in_patient"],
                "  [broad component response]" if om_flag else ""))
            ...
            axB.set_title("B. display crop (%s)  score=%.3f (thr=%s)  area-%.0f%%" % (
                DISPLAY_BBOX_MODE, float(row["max_padim_score"]), THRESHOLD_P95, red * 100))
            # C reference
            ...
            fig.suptitle("%s | %s | %s%s" % (cid, role, safe_id, "  [OVERMERGE]" if om_flag else ""), fontsize=10)
            fig.tight_layout()
    ...
    summary = {
        "mode": "s3_prototype_cards_v2", "display_bbox_mode": DISPLAY_BBOX_MODE,
    ...
    print("[run-cards v2] 완료. cards_ok=%d errors=%d overmerge=%d -> %s"
...
def mode_dry_run():
    ...
    chk("v2 DONE 부재", not os.path.exists(os.path.join(OUT_DIR, "DONE.json")), "(존재시 BLOCKED)")
...
def mode_selftest():
    ...
    # display bbox B2 + margin + clip + area + reduction
    ...
    # max-score patch matching + 동점 union 우선
    ...
    # schema
    expect("CARD_JSON v2 schema 39열", len(CARD_JSON_FIELDS) == 39)
    expect("INDEX v2 schema 19열", len(INDEX_FIELDS) == 19)
    expect("새 JSON 필드 포함", all(k in CARD_JSON_FIELDS for k in
    ...
    src_dry = inspect.getsource(mode_dry_run); src_plan = inspect.getsource(mode_plan_only)
...
def build_parser():
    p = argparse.ArgumentParser(description="Explanation Card S3 v2 card 생성기 (local display bbox; 가드 필수).")
