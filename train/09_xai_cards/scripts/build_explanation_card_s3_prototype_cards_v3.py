...
# -*- coding: utf-8 -*-
"""
Explanation Card S3 v3 : minor layout fix (Panel A/B title overlap 해소)

기준:
  - reports/explanation_cards/s3_card_v3_minor_layout_preflight_v1.md (PASS)
  - v2 스크립트 build_explanation_card_s3_prototype_cards_v2.py (참고/보존)

v3 변경 (layout 만):
  - Panel A title 단축: "A. Whole slice | rank=R | bin=position_bin"
    (v2의 [broad component response] 제거)
  - Panel B title 단축: "B. Local crop | score=xx.xx / thr=14.09"
    (v2의 display_bbox_mode 긴 문자열 + area-N% 제거)
  - overmerge caution: Panel A title → axA.text annotation(하단 좌측)으로 이동
    EN: "Broad component: red box marks the highest-score local region."
    KO: "광역 component: 빨간 박스는 최고 점수 국소 영역입니다."
  - suptitle: safe_id 앞 16자+... 축약, [OVERMERGE] 태그 제거
  - v3 output root: s3_prototype_cards_v3/ (v1/v2 무수정)

변경하지 않은 것:
  - B2 display_bbox 로직 (max-score patch bbox + margin32)
  - component_union_bbox + display_bbox 둘 다 JSON 보존
  - overmerge_flag / overmerge_level / overmerge_reason 기준
  - JSON numeric casting (int/float/bool 명확 캐스팅)
  - explanation_text / fp_caution_text / overmerge_caution_text 내용
  - diagnostic_terms_blocked list / diagnostic_guard_passed bool
  - Panel A/B/C/D 4-panel 구조
  - same-bin normal reference crop 선택 로직
  - z-context logic
  - threshold 14.0921
  - manifest 24건
  - S2 component 병합 로직, B5 local-radius (범위 밖)
...
가드:
  ...
  - manifest 24행만 대상(full/all 없음). holdout 교집합 0 assert.
...
# explanation_text / fp_caution_text용 (v2와 동일)
...
# v3 신규: Panel A annotation 전용 (title에서 분리된 짧은 문구)
...
CARD_JSON_FIELDS = (
    ...
    "max_score_slice_index", "z_span", "component_union_bbox", "display_bbox",
    ...
    "overmerge_reason", "overmerge_caution_text", "patch_count", "max_padim_score",
    ...
    "source_manifest", "stage_split_safety_flag", "diagnostic_terms_blocked",
...
INDEX_FIELDS = (
    ...
    "display_bbox_area_reduction_ratio", "card_png_path", "card_json_path", "status", "error",
)
...
COMPONENT_CSV = os.path.join(CAND_ROOT, "component_candidates.csv")
...
# ----------------------------------------------------------------------------
# v3 신규: 타이틀/suptitle 빌더 (selftest 대상)
# ----------------------------------------------------------------------------
...
def pick_max_score_patch(patches, max_score_slice, max_score, union_bbox):
    ...
    return min(pool, key=lambda p: ((p["y0"] + p["y1"]) / 2 - ucy) ** 2 + ((p["x0"] + p["x1"]) / 2 - ucx) ** 2)
...
def load_proto_patches(safe_ids):
    """patch_candidates.csv 를 prototype safe_id 만 streaming 수집 (전수 로드 금지)."""
    want = set(safe_ids)
    out = {}
...
def _generate_cards(out_dir):
    ...
    patches_by_key = load_proto_patches(sids)
    ...
    for row in rows:
        ...
        try:
            ...
            pc = int(row["patch_count"]); zsp = int(row["z_span"])
            om_flag, om_level, om_reason = overmerge_eval(pc, zsp, comp_area)
            ...
            axA = axes[0, 0]; axA.imshow(base, cmap="gray"); axA.axis("off")
            # v3: overmerge caution 제거된 짧은 title
            axA.set_title(build_panel_a_title(zc, row["position_bin"], row["rank_in_patient"]))
            # union bbox: 얇은 dashed 연한색
            ...
            # v3: overmerge caution → Panel A 내부 annotation으로 이동
            if om_flag:
            ...
            if ref_imgs:
                ...
                axC.imshow(concat)
            # D z-context (display bbox)
            axD = axes[1, 1]; axD.axis("off"); axD.set_title("D. z-context %s" % zs)
            if zs:
            ...
            card = {
                ...
                "rank_in_patient": int(row["rank_in_patient"]), "position_bin": row["position_bin"],
                "slice_index_min": int(row["slice_index_min"]), "slice_index_max": int(row["slice_index_max"]),
                "max_score_slice_index": int(row["max_score_slice_index"]), "z_span": zsp,
                "component_union_bbox": [int(v) for v in union_bbox], "display_bbox": [int(v) for v in disp_c],
                "display_bbox_mode": DISPLAY_BBOX_MODE, "display_bbox_margin": int(DISPLAY_MARGIN),
                "display_bbox_area": int(disp_area), "component_bbox_area": int(comp_area),
                "display_bbox_area_reduction_ratio": float(red),
                "overmerge_flag": bool(om_flag), "overmerge_level": om_level, "overmerge_reason": om_reason,
                "overmerge_caution_text": (OVERMERGE_CAUTION_KO + " " + OVERMERGE_CAUTION_EN) if om_flag else "",
                "patch_count": pc, "max_padim_score": float(row["max_padim_score"]),
                "mean_padim_score": float(row["mean_padim_score"]), "threshold": float(THRESHOLD_P95),
                "threshold_type": THRESHOLD_TYPE,
                "roi_0_0_patch_ratio_mean": float(row["roi_0_0_patch_ratio_mean"]) if row["roi_0_0_patch_ratio_mean"] else None,
                "central_peripheral": row["central_peripheral"],
                "central_distance_ratio_mean": float(row["central_distance_ratio_mean"]) if row["central_distance_ratio_mean"] else None,
            ...
            index_rows.append({
                ...
                "safe_id": safe_id, "component_id": row["component_id"],
                "rank_in_patient": int(row["rank_in_patient"]), "position_bin": row["position_bin"],
                "max_padim_score": float(row["max_padim_score"]), "threshold": float(THRESHOLD_P95),
                "z_span": zsp, "patch_count": pc, "display_bbox_mode": DISPLAY_BBOX_MODE,
                "overmerge_flag": bool(om_flag), "overmerge_level": om_level,
                "display_bbox_area_reduction_ratio": float(red),
            ...
            index_rows.append({
                ...
                "max_padim_score": row.get("max_padim_score", ""), "threshold": float(THRESHOLD_P95),
                "z_span": row.get("z_span", ""), "patch_count": row.get("patch_count", ""),
                "display_bbox_mode": DISPLAY_BBOX_MODE, "overmerge_flag": "", "overmerge_level": "",
                "display_bbox_area_reduction_ratio": "", "card_png_path": "", "card_json_path": "",
    ...
    summary = {
        "mode": "s3_prototype_cards_v3", "display_bbox_mode": DISPLAY_BBOX_MODE,
        "n_manifest_rows": len(rows), "n_cards_ok": n_ok, "n_errors": len(errors),
        "n_overmerge_flagged": sum(1 for r in index_rows if r.get("overmerge_flag") is True),
        "threshold": float(THRESHOLD_P95), "threshold_type": THRESHOLD_TYPE,
    ...
    print("[run-cards v3] 완료. cards_ok=%d errors=%d overmerge=%d -> %s"
          % (n_ok, len(errors), summary["n_overmerge_flagged"], out_dir))
    return EXIT_OK
...
def mode_dry_run():
    ...
    chk("v3 output DONE 부재", not os.path.exists(os.path.join(OUT_DIR, "DONE.json")), "(존재시 BLOCKED)")
...
def mode_plan_only():
    ...
    rows = load_manifest()
    hp, hs = load_holdout_denylist()
    pids = set(r["patient_id"].strip() for r in rows); sids = set(r["safe_id"].strip() for r in rows)
    print("  manifest rows:", len(rows), "(기대 %d)" % N_EXPECTED_ROWS)
    print("  patients:", len(sids), "(기대 %d)" % N_EXPECTED_PATIENTS)
    try:
    ...
    ok = (len(rows) == N_EXPECTED_ROWS and len(sids) == N_EXPECTED_PATIENTS
...
def mode_selftest():
    ...
    expect("display bbox B2+margin32", disp == [68, 68, 164, 164])
    expect("bbox clip 범위", clip_bbox([-10, -10, 1000, 1000], 512, 512) == [0, 0, 512, 512])
    expect("display area 계산", bbox_area([68, 68, 164, 164]) == 96 * 96)
    expect("component area 계산", bbox_area([0, 0, 200, 200]) == 40000)
    expect("area reduction ratio", area_reduction_ratio(40000, 9216) == round(1 - 9216 / 40000, 4))
    expect("reduction union0 -> 0", area_reduction_ratio(0, 100) == 0.0)

    # --- overmerge 기준 ---
    f1, l1, _ = overmerge_eval(4160, 96, 101376)
    expect("extreme_union", f1 and l1 == "extreme_union")
    f2, l2, _ = overmerge_eval(300, 35, 30000)
    expect("large_union(z>=30 or area>=25000)", f2 and l2 == "large_union")
    f3, l3, _ = overmerge_eval(50, 5, 5000)
    expect("non-overmerge", (not f3) and l3 == "none")
    f4, l4, _ = overmerge_eval(600, 10, 5000)
    expect("large_union(pc>=500)", f4 and l4 == "large_union")

    # --- max-score patch ---
    patches = [{"slice": 10, "score": 30.0, "y0": 0, "x0": 0, "y1": 32, "x1": 32, "ps": 32},
               {"slice": 10, "score": 30.0, "y0": 200, "x0": 200, "y1": 232, "x1": 232, "ps": 32},
               {"slice": 11, "score": 31.0, "y0": 0, "x0": 0, "y1": 32, "x1": 32, "ps": 32}]
    mp = pick_max_score_patch(patches, 10, 30.0, [0, 0, 100, 100])
    expect("max-score patch slice+score 매칭", mp is not None and mp["slice"] == 10)
    expect("동점 union 내부 우선", mp["y0"] == 0 and mp["x0"] == 0)
    expect("매칭 실패 None", pick_max_score_patch(patches, 99, 30.0, [0, 0, 100, 100]) is None)

    # --- 금지어 / 텍스트 ---
    expect("forbidden term 검출", "adenocarcinoma" in scan_forbidden_terms("x adenocarcinoma y"))
    expect("clean 통과", scan_forbidden_terms("broad component response 광역 component 반응") == [])
    ...
    nt = build_explanation_text(nrow, False); lt = build_explanation_text(lrow, True)
    expect("normal_control 문구", "Normal control" in nt and "정상 control FP" in nt)
    expect("lesion_candidate 문구", "Stage1-dev candidate" in lt)
    expect("overmerge caution 포함(flag)", "광역 component 반응" in lt and "Broad component response" in lt)
    expect("overmerge 문구 금지어 없음", scan_forbidden_terms(lt) == [] and "diffuse" not in lt.lower())
    expect("normal fp_caution", "정상 control" in build_fp_caution_text(nrow, False))
    expect("overmerge fp_caution", "광역" in build_fp_caution_text(lrow, True))

    # --- reference / z / window / contour ---
    ...
    expect("reference 같은 bin/최대3", len(sel) == 3 and all(r["position_bin"] == "upper_central" for r in sel))
    expect("z-context 클립", z_context_slices(0, 5) == [0, 1] and z_context_slices(10, 50) == [9, 10, 11])
    ...
    expect("mask contour 경계만", bool(cont[0, 0]) and (not bool(cont[1, 1])))

    # --- schema ---
    expect("CARD_JSON schema 39열", len(CARD_JSON_FIELDS) == 39)
    expect("INDEX schema 19열", len(INDEX_FIELDS) == 19)
    expect("필수 JSON 필드 포함", all(k in CARD_JSON_FIELDS for k in
           ("component_union_bbox", "display_bbox", "display_bbox_mode", "display_bbox_area",
            "component_bbox_area", "display_bbox_area_reduction_ratio", "overmerge_flag",
            "overmerge_level", "diagnostic_guard_passed")))

    # --- v3 신규: Panel title 빌더 ---
    pa = build_panel_a_title(512, "lower_peripheral", 9)
    expect("Panel A title ≤50자", len(pa) <= 50)
    expect("Panel A title broad/광역 없음",
           "broad" not in pa.lower() and "광역" not in pa and "caution" not in pa.lower())
    pb = build_panel_b_title(999.99, THRESHOLD_P95)
    expect("Panel B title ≤60자", len(pb) <= 60)
    expect("Panel B title에 DISPLAY_BBOX_MODE 없음", DISPLAY_BBOX_MODE not in pb)

    # --- v3 신규: overmerge annotation ---
    ann = _overmerge_annotation_text()
    expect("annotation Broad component 포함", "Broad component" in ann)
    expect("annotation 광역 component 포함", "광역 component" in ann)
    expect("annotation 금지어 0", scan_forbidden_terms(ann) == [])
    expect("annotation diffuse/focal 미포함", "diffuse" not in ann.lower() and "focal" not in ann.lower())

    # --- v3 신규: suptitle 빌더 ---
    long_sid = "MSD_Lung_MSD_lung_054__f513717b03"
    st = build_suptitle("MSD_lung_054__c1", ROLE_LESION, long_sid)
    expect("suptitle OVERMERGE 태그 없음", "OVERMERGE" not in st)
    expect("suptitle 대괄호 없음", "[" not in st)
    sh = _short_safe_id(long_sid)
    expect("safe_id 축약 16자+...=19자", len(sh) == 19 and sh.endswith("..."))
    sh2 = _short_safe_id("short")
    expect("safe_id 짧으면 그대로", sh2 == "short")

    # --- v3 신규: output root 분리 ---
    expect("v3 output root != v2", OUT_DIR != V2_OUT_DIR)
    expect("v3 output root에 v3 포함", "v3" in OUT_DIR)

    # --- v3 신규: v2 output 보존 ---
    expect("v2 output 보존(V2_OUT_DIR 존재)", os.path.isdir(V2_OUT_DIR))
    ...
    expect("run-cards 실제 연결", "_generate_cards(OUT_DIR)" in src_run)
    ...
    expect("display_bbox 사용(union crop 폐기)", "ct[zc, disp_c[0]:disp_c[2], disp_c[1]:disp_c[3]]" in src_gen)
    expect("union+display 둘다 JSON", "component_union_bbox" in src_gen and "display_bbox" in src_gen)
    expect("int 캐스팅", "int(row[\"slice_index_min\"])" in src_gen and "int(row[\"patch_count\"])" in src_gen)
    expect("float 캐스팅", "float(row[\"max_padim_score\"])" in src_gen)
    expect("bool overmerge_flag", "bool(om_flag)" in src_gen)
    expect("diagnostic_terms_blocked list 유지", "list(FORBIDDEN_TERMS)" in src_gen)
    expect("lesion GT mask 미사용", "lesion_mask" not in src_gen)
    expect("v1 출력 미수정", "V1_OUT_DIR" not in src_gen)
    expect("전체 산출물 기록", all(k in src_gen for k in ("cards_png", "cards_json", "index_cards.csv",
            "runtime_summary.json", "errors.csv", "DONE.json")))

    # --- v3 소스 검사: title/annotation 분리 ---
    expect("gen: axA.set_title에 broad 없음", "broad component response" not in src_gen.lower())
    expect("gen: suptitle에 [OVERMERGE] 없음", "[OVERMERGE]" not in src_gen)
    expect("gen: build_panel_a_title 호출", "build_panel_a_title(" in src_gen)
    expect("gen: build_panel_b_title 호출", "build_panel_b_title(" in src_gen)
    expect("gen: build_suptitle 호출", "build_suptitle(" in src_gen)
    expect("gen: _overmerge_annotation_text 호출", "_overmerge_annotation_text()" in src_gen)
    expect("gen: axA.text annotation 존재", "axA.text(" in src_gen)
    expect("bare guard(main src)", "EXIT_BLOCKED" in inspect.getsource(main))
    expect("dry: v2 output 보존 확인", "V2_OUT_DIR" in src_dry)
...
def build_parser():
    p = argparse.ArgumentParser(description="Explanation Card S3 v3 card 생성기 (layout fix; 가드 필수).")
