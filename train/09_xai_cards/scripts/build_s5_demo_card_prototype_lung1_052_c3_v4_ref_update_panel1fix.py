"""
build_s5_demo_card_prototype_lung1_052_c3_v4_ref_update_panel1fix.py

목적:
v4_ref_update 카드 Panel 1 aspect/layout fix preflight(PASS, option B) 결과를 반영하는
신규 카드 스크립트. 기존 v4_ref_update / v3d 산출물은 수정하지 않는다(신규 파일 + 신규 output root).
...
Panel 2/3/4: unchanged (v4_ref_update 와 동일).
...
VERSION            = "v4_ref_update_aspectfix"
...
# 신규 render 출력 root (다음 단계)
OUTPUT_ROOT    = EC / "s5_demo_card_prototype_lung1_052_c3_v4_ref_update_aspectfix"
CARDS_PNG_DIR  = OUTPUT_ROOT / "cards_png"
CARDS_JSON_DIR = OUTPUT_ROOT / "cards_json"
OUT_PNG        = CARDS_PNG_DIR  / f"{CASE_ID}_s5_demo_prototype_v4_ref_update_aspectfix.png"
OUT_JSON       = CARDS_JSON_DIR / f"{CASE_ID}_s5_demo_prototype_v4_ref_update_aspectfix.json"
OUT_INDEX_CSV  = OUTPUT_ROOT / "index_cards_v4_ref_update_aspectfix.csv"
OUT_RUNTIME    = OUTPUT_ROOT / "runtime_summary_v4_ref_update_aspectfix.json"
OUT_ERRORS     = OUTPUT_ROOT / "errors.csv"
OUT_DONE       = OUTPUT_ROOT / "DONE.json"

# 이번 단계(static drycheck) 리포트 root
DRYCHECK_ROOT = EC / "lung1_052_s5_demo_card_v4_ref_update_panel1fix_script_static_drycheck"
DRY_MD        = DRYCHECK_ROOT / "static_drycheck_v4_ref_update_panel1fix.md"
DRY_JSON      = DRYCHECK_ROOT / "static_drycheck_v4_ref_update_panel1fix.json"
DRY_PLAN_CSV  = DRYCHECK_ROOT / "script_plan_v4_ref_update_panel1fix.csv"
DRY_TILE_CSV  = DRYCHECK_ROOT / "panel1_tile_layout_v4_ref_update_panel1fix.csv"
DRY_CROP_CSV  = DRYCHECK_ROOT / "clean_crop_source_v4_ref_update_panel1fix.csv"
DRY_SAFE_CSV  = DRYCHECK_ROOT / "safety_check_v4_ref_update_panel1fix.csv"
...
def selftest() -> bool:
    ...
    t("version_aspectfix", VERSION == "v4_ref_update_aspectfix")
    t("output_root_aspectfix", "v4_ref_update_aspectfix" in str(OUTPUT_ROOT))
...
def static_drycheck() -> bool:
    ...
    report = {
        ...
        "stage": "lung1_052_v4_ref_update_panel1fix_script_static_drycheck",
        "this_stage_render": 0, "this_stage_ct_load": 0,
        "checks": rows, "n_pass": n_pass, "n_fail": n_fail,
        "script": "scripts/build_s5_demo_card_prototype_lung1_052_c3_v4_ref_update_panel1fix.py",
    ...
    md = [
        f"# {CASE_ID} S5 demo card v4-ref Panel 1 clean-tile fix — SCRIPT static drycheck",
        f"date: {date.today()}",
