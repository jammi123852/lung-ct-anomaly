...
LAYOUT_SELECTED    = "DYNAMIC_REF_4PANEL_FINAL_AB_PANEL1_CONTEXTCROP_POSITIONBOX"
VERSION            = "efficientnet_dynamic_ref_contextbox_layoutfix"
...
# ---- Panel1 context-crop + 80mm position-box 표시 옵션 ----
CONTEXT_FACTOR = 2.0   # Panel1 tile = 80mm 위치의 CONTEXT_FACTOR 배 맥락 crop + 안쪽 80mm box
BANK_ROOT = pathlib.Path(
    "/home/jinhy/project/lung-ct-anomaly/outputs/position-aware-padim-v1/reports"
    "/dynamic_normal_reference_bank_three_patients_v1")
CAND_CT_PATH = pathlib.Path(
    "/mnt/c/Users/jinhy/Desktop/NSCLC_MSD_padim_test_ready_roi0_0_ts_lung_raw_no_dilate_usable_only_v1"
...
OUTPUT_ROOT    = REPORTS_ROOT / "efficientnet_s5_card_lung1_052_c3_dynamic_ref_contextbox_layoutfix"
CARDS_PNG_DIR  = OUTPUT_ROOT / "cards_png"
CARDS_JSON_DIR = OUTPUT_ROOT / "cards_json"
OUT_PNG        = CARDS_PNG_DIR  / f"{CASE_ID}_efficientnet_dynamic_ref_contextbox_layoutfix.png"
OUT_JSON       = CARDS_JSON_DIR / f"{CASE_ID}_efficientnet_dynamic_ref_contextbox_layoutfix.json"
OUT_INDEX_CSV  = OUTPUT_ROOT / "index_cards_dynamic_ref_contextbox_layoutfix.csv"
OUT_RUNTIME    = OUTPUT_ROOT / "runtime_summary_dynamic_ref_contextbox_layoutfix.json"
...
DRYCHECK_ROOT = REPORTS_ROOT / "efficientnet_s5_card_lung1_052_c3_dynamic_ref_contextbox_static_drycheck"
DRY_MD        = DRYCHECK_ROOT / "static_drycheck_dynamic_ref_contextbox.md"
DRY_JSON      = DRYCHECK_ROOT / "static_drycheck_dynamic_ref_contextbox.json"
DRY_PLAN_CSV  = DRYCHECK_ROOT / "script_plan_dynamic_ref_contextbox.csv"
DRY_TILE_CSV  = DRYCHECK_ROOT / "panel1_tile_layout_dynamic_ref_contextbox.csv"
DRY_CROP_CSV  = DRYCHECK_ROOT / "panel1_dynamic_source_dynamic_ref_contextbox.csv"
DRY_SAFE_CSV  = DRYCHECK_ROOT / "safety_check_dynamic_ref_contextbox.csv"
...
PANEL1_SUBTITLE_EN = ("Dynamic normal references matched by bilateral lung frame "
                      "(lung_z_pct + lung-bbox relative y/x); each tile shows a 2x context crop "
                      "with the 80mm target position as a box; this is not same-z matching.")
PANEL1_SUBTITLE_KO = ("정상 reference는 양쪽 폐(bilateral) 기준 폐 내부 상대 위치로 동적 매칭했고, "
                      "각 tile은 2배 맥락 crop 안에 원래 봐야 할 80mm 위치를 box로 표시합니다(배율 정규화 유지). "
                      "같은 z 번호 정합이 아닙니다.")
...
def render_card() -> None:
    if not (ALLOW_CARD_RENDER and ALLOW_SOURCE_IMAGE_READ and ALLOW_CT_LOAD and ALLOW_PNG_WRITE):
        _abort("render requires ALLOW_CARD_RENDER=1 + ALLOW_SOURCE_IMAGE_READ=1 + ALLOW_CT_LOAD=1 + ALLOW_PNG_WRITE=1 "
               f"(current: render={ALLOW_CARD_RENDER}, src={ALLOW_SOURCE_IMAGE_READ}, ct={ALLOW_CT_LOAD}, png={ALLOW_PNG_WRITE})")
    ...
    def make_clean_tile(role):
        """source slice 에서 CONTEXT_FACTOR 배 '맥락 crop' + 원래 봐야 할 80mm 위치를 box 로 표시.
        candidate = read-only CT slice(z51) / ref = dynamic bank PNG slice. contain-fit into TILE_PX.
        -> 작은 80mm crop 만 보면 종격동 등이 흰색으로 차서 해석 어려운 문제를, 맥락+위치박스로 해결."""
        import numpy as np
        if role == "candidate":
            ct = np.load(str(CAND_CT_PATH), mmap_mode="r")
            sl = window_hu_to_uint8(np.asarray(ct[CT_LOCAL_Z]))
            inner = list(CAND_CROP_BBOX_NAT); box_color = (255, 70, 70, 255)
        else:
            rm = DYN_REF_BY_ALIAS.get(role, {})
            z = int(rm["selected_local_z"]); inner = list(rm["crop_bbox_native"])
            png_path = BANK_ROOT / role / "slices_png" / f"z_{z:03d}.png"
            sl = np.array(Image.open(str(png_path)).convert("L")); box_color = (255, 220, 0, 255)
        H, W = sl.shape
        y0, x0, y1, x1 = inner; cpx = y1 - y0
        cyc = (y0 + y1) // 2; cxc = (x0 + x1) // 2
        bpx = int(round(cpx * CONTEXT_FACTOR)); half = bpx // 2
        by0 = max(0, min(cyc - half, H - bpx)); bx0 = max(0, min(cxc - half, W - bpx))
        crop = sl[by0:by0 + bpx, bx0:bx0 + bpx]
        src = Image.fromarray(crop, mode="L").convert("RGBA")
        ...
        d.rectangle([ox + (x0 - bx0) * s, oy + (y0 - by0) * s,
                     ox + (x1 - bx0) * s, oy + (y1 - by0) * s], outline=box_color, width=2)
        if role == "candidate":  # 안쪽에 EfficientNet peak box(주황) 추가
            d.rectangle([ox + (EFF_PEAK_BBOX[1] - bx0) * s, oy + (EFF_PEAK_BBOX[0] - by0) * s,
                         ox + (EFF_PEAK_BBOX[3] - bx0) * s, oy + (EFF_PEAK_BBOX[2] - by0) * s],
    ...
    schema["panel1_display"] = "context_crop_2x_with_80mm_position_box"
    ...
    with open(OUT_RUNTIME, "w", encoding="utf-8") as f:
        json.dump({"case_id": CASE_ID, "version": VERSION, "elapsed_sec": round(elapsed, 2),
                   "ct_load_used": True, "ct_volumes_loaded": 1,
    ...
    with open(OUT_DONE, "w", encoding="utf-8") as f:
        json.dump({"done": True, "case_id": CASE_ID, "version": VERSION,
                   ...
                   "same_z_matching": SAME_Z_MATCHING,
                   "panel1_display": "context_crop_2x_with_80mm_position_box",
                   "ct_load_used": True, "score_recomputed": False,
...
def selftest() -> bool:
    ...
    t("version_dynamic_ref_contextbox", VERSION == "efficientnet_dynamic_ref_contextbox_layoutfix")
    t("output_root_contextbox", "efficientnet_s5_card_lung1_052_c3_dynamic_ref_contextbox_layoutfix" in str(OUTPUT_ROOT))
    t("not_overwrite_layoutfix",
      str(OUTPUT_ROOT) != str(REPORTS_ROOT / "efficientnet_s5_card_lung1_052_c3_v4_20_layoutfix"))
    t("not_overwrite_dynamic_ref_layoutfix",
      str(OUTPUT_ROOT) != str(REPORTS_ROOT / "efficientnet_s5_card_lung1_052_c3_dynamic_ref_layoutfix"))
