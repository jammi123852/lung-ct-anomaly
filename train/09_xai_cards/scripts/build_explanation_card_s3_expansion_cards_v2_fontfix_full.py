...
# -*- coding: utf-8 -*-
"""
Explanation Card S3 Expansion v2 Font-Fix Full (전체 300장)

기준:
  - reports/explanation_cards/s3_font_fix_full_preflight_v1.md (PASS)
  - smoke script: build_explanation_card_s3_expansion_cards_v2_fontfix_smoke.py (보존 / 수정 금지)
  - 기존 expansion v1: build_explanation_card_s3_expansion_cards_v1.py (보존 / 수정 금지)

smoke → full 변경점:
  - SMOKE_IDS 필터 제거 → 전체 300행 처리
  - N_EXPECTED_ROWS = 300 (smoke N_SMOKE_EXPECTED=6 대체)
  - OUT_DIR: s3_expansion_cards_v2_fontfix/ (smoke output 보존)
  - resolve_role() 추가: row["role"] → row["prototype_role"] → error (명시적 처리)
  - build_explanation_text / build_fp_caution_text: resolve_role() 사용
  - CARD_JSON_FIELDS / INDEX_FIELDS: "role" 필드 추가
  - mode string: s3_expansion_cards_v2_fontfix_full
  - selftest: full 기준 항목으로 교체

변경하지 않은 것:
  - 한글 폰트 설정 (resolve_korean_font)
...
가드:
  ...
  - smoke output 보존 (수정/삭제/덮어쓰기 금지).
  - full 300 전용 (SMOKE_IDS 미사용).
...
THRESHOLD_TYPE = "p95"
N_EXPECTED_ROWS = 300          # 전체 300장 기대 (smoke: N_SMOKE_EXPECTED=6)
...
# role 필드 추가: smoke에는 prototype_role만 있었음
CARD_JSON_FIELDS = (
    "expansion_case_id", "role", "prototype_role", "patient_id", "safe_id", "component_id",
...
# ----------------------------------------------------------------------------
# role 해결 함수 (full 신규)
# 우선순위: row["role"] → row["prototype_role"] → RuntimeError
# ----------------------------------------------------------------------------
def resolve_role(row):
    """manifest row에서 역할 해결.
    우선순위: row["role"] → row["prototype_role"] → RuntimeError.
    role 필드 부재(빈 문자열 포함) 시 prototype_role로 fallback.
    둘 다 없으면 명시적 error.
    """
    r = (row.get("role") or "").strip()
    if r:
        return r
    pr = (row.get("prototype_role") or "").strip()
    if pr:
        return pr
    raise RuntimeError(
        "role 및 prototype_role 둘 다 없음: expansion_case_id=%s" % row.get("expansion_case_id", "?")
    )
...
def build_explanation_text(row, overmerge_flag):
    rd = resolve_role(row)  # role 해결 (role → prototype_role → error)
    if rd == ROLE_NORMAL:
...
def build_fp_caution_text(row, overmerge_flag):
    parts = []
    if resolve_role(row) == ROLE_NORMAL:  # role 해결 (role → prototype_role → error)
...
# ----------------------------------------------------------------------------
# 카드 생성 (--run-cards --confirm-generate)
# ----------------------------------------------------------------------------
def _generate_cards(out_dir):
    ...
    # ── full 300 확인 (smoke 필터 없음) ───────────────────────────
    if len(rows_all) != N_EXPECTED_ROWS:
        sys.stderr.write("[BLOCKED] manifest %d행 기대 %d행\n" % (len(rows_all), N_EXPECTED_ROWS))
        return EXIT_BLOCKED

    ref_rows = load_reference_crop_rows()
    pids = set(r["patient_id"].strip() for r in rows_all)
    sids = set(r["safe_id"].strip() for r in rows_all)
    ...
    for row in rows_all:
        ...
        role = resolve_role(row)  # role 해결 (role → prototype_role → error)
        ...
        try:
            ...
            err_role = ""
            try:
                err_role = resolve_role(row)
            except Exception:
                err_role = row.get("role", row.get("prototype_role", ""))
    ...
    summary = {
        "mode": "s3_expansion_cards_v2_fontfix_full",
        "display_bbox_mode": DISPLAY_BBOX_MODE,
        "n_full_expected": N_EXPECTED_ROWS,
        "n_manifest_rows": len(rows_all),
        "n_cards_ok": n_ok,
        "n_errors": len(errors),
    ...
    print("[run-cards full v2] 완료. cards_ok=%d errors=%d font=%s -> %s"
...
def mode_dry_run():
    ...
    chk("smoke output 보존(존재)", os.path.isdir(SMOKE_OUT_DIR))
    ...
    chk("full output DONE 미존재(충돌없음)", not os.path.exists(os.path.join(OUT_DIR, "DONE.json")),
        "(존재시 BLOCKED)")
    ...
    print("  [PLAN] full 대상 %d장" % N_EXPECTED_ROWS)
...
def mode_plan_only():
    print("[MODE] --plan-only (300장 CT/mask path 존재만; npy 미열람)")
    rows_all = load_manifest()
    if len(rows_all) != N_EXPECTED_ROWS:
        print("  [BLOCKED] manifest 행수 %d (기대 %d)" % (len(rows_all), N_EXPECTED_ROWS))
        return EXIT_BLOCKED
    hp, hs = load_holdout_denylist()
    pids = set(r["patient_id"].strip() for r in rows_all)
    sids = set(r["safe_id"].strip() for r in rows_all)
    ...
    for r in rows_all:
        ...
        if os.path.isfile(safe_path(resolve_mask_path(sid, role))):
            mask_ok += 1
    print("  manifest 행수:", len(rows_all), "(기대 %d)" % N_EXPECTED_ROWS)
    print("  CT path 존재:", ct_ok, "/", len(rows_all))
    print("  mask path 존재:", mask_ok, "/", len(rows_all))
    ...
    print("  smoke output 보존:", os.path.isdir(SMOKE_OUT_DIR))
    print("  HOLD_IDS:", sorted(HOLD_IDS))
    ok = (len(rows_all) == N_EXPECTED_ROWS
          and ct_ok == len(rows_all) and mask_ok == len(rows_all) and inter == 0)
...
def mode_selftest():
    ...
    # ── full 전용 확인 ──────────────────────────────────────────
    expect("F1. N_EXPECTED_ROWS == 300", N_EXPECTED_ROWS == 300)

    expect("F2. HOLD_IDS 수 == 3", len(HOLD_IDS) == 3)
    expect("F2b. HOLD_IDS 3건 포함",
           all(h in HOLD_IDS for h in ("LUNG1-284__c1", "LUNG1-220__c3", "LUNG1-402__c1")))

    expect("F3. SMOKE_IDS 미정의 (smoke 필터 제거)", not hasattr(sys.modules[__name__], "SMOKE_IDS"))

    expect("F4. FONT_CANDIDATES 비어 있지 않음", len(FONT_CANDIDATES) > 0)
    expect("F5. PRIMARY = malgun.ttf", "malgun.ttf" in FONT_CANDIDATES[0])
    expect("F6. fallback 2종 존재", len(FONT_CANDIDATES) >= 2)

    src_font = inspect.getsource(resolve_korean_font)
    expect("F6a. addfont 호출", "addfont(" in src_font)
    expect("F6b. get_name() 호출", "get_name()" in src_font)
    expect("F6c. RuntimeError 발생 (전부 실패 시)", "RuntimeError" in src_font)
    expect("F6d. DejaVu로 조용히 넘어가지 않음", "DejaVu" not in src_font)

    expect("F7. N_EXPECTED_ROWS == 300", N_EXPECTED_ROWS == 300)
    expect("F7b. SMOKE_IDS 미정의 확인", not hasattr(sys.modules[__name__], "SMOKE_IDS"))

    expect("F8. OUT_DIR에 fontfix_smoke 없음", "fontfix_smoke" not in OUT_DIR)
    expect("F8b. OUT_DIR에 fontfix 포함", "fontfix" in OUT_DIR)

    expect("F9. V1_EXPANSION_OUT_DIR 존재", os.path.isdir(V1_EXPANSION_OUT_DIR))
    expect("F9b. SMOKE_OUT_DIR 존재", os.path.isdir(SMOKE_OUT_DIR))

    src_gen = inspect.getsource(_generate_cards)
    expect("F10a. _generate_cards에 SMOKE_IDS 필터 없음", "SMOKE_IDS" not in src_gen)
    expect("F10b. N_EXPECTED_ROWS 체크 in _generate_cards", "N_EXPECTED_ROWS" in src_gen)
    expect("F10c. stage2_holdout FORBIDDEN_PATH_TOKENS에 포함", "stage2_holdout" in FORBIDDEN_PATH_TOKENS)

    expect("F11a. resolve_korean_font() 호출", "resolve_korean_font()" in src_gen)
    expect("F11b. rcParams font.family 설정", "rcParams['font.family']" in src_gen)

    expect("F12a. selected_font_path in summary", '"selected_font_path"' in src_gen)
    expect("F12b. selected_font_family in summary", '"selected_font_family"' in src_gen)
    expect("F12c. mode = v2_fontfix_full", '"s3_expansion_cards_v2_fontfix_full"' in src_gen)

    # ── role resolution 확인 ──────────────────────────────────────
    expect("R1. resolve_role (role 있을 때)",
           resolve_role({"role": "lesion_candidate", "prototype_role": "expansion"}) == "lesion_candidate")
    expect("R1b. resolve_role normal_control",
           resolve_role({"role": "normal_control"}) == "normal_control")
    expect("R2. resolve_role fallback prototype_role",
           resolve_role({"role": "", "prototype_role": "expansion"}) == "expansion")
    expect("R2b. resolve_role fallback None role",
           resolve_role({"prototype_role": "expansion"}) == "expansion")

    blk_role = False
    try:
        resolve_role({"expansion_case_id": "test__c1"})
    except RuntimeError:
        blk_role = True
    expect("R3. resolve_role 둘 다 없으면 RuntimeError", blk_role)

    expect("R4. CARD_JSON_FIELDS에 role 포함", "role" in CARD_JSON_FIELDS)
    expect("R5. INDEX_FIELDS에 role 포함", "role" in INDEX_FIELDS)

    src_expl = inspect.getsource(build_explanation_text)
    src_fp = inspect.getsource(build_fp_caution_text)
    expect("R6. build_explanation_text uses resolve_role", "resolve_role(" in src_expl)
    expect("R7. build_fp_caution_text uses resolve_role", "resolve_role(" in src_fp)
    ...
    expect("3. output DONE 가드", "DONE.json 존재" in src_gen and "잔여 산출물" in src_gen)
    ...
    expect("31b. OUT_DIR != SMOKE_OUT_DIR", OUT_DIR != SMOKE_OUT_DIR)
    expect("31c. OUT_DIR에 fontfix_smoke 없음", "fontfix_smoke" not in OUT_DIR)
    ...
    required_idx = ("expansion_case_id", "role", "prototype_role", "overmerge_flag", "apex_caution",
                    "internal_use_only", "font_fix_required_before_external_share",
                    "card_png_path", "card_json_path", "status", "error")
    ...
    expect("35c. runtime_summary mode full", '"s3_expansion_cards_v2_fontfix_full"' in src_gen)
    ...
    expect("extra. threshold THRESHOLD_P95 유지", "THRESHOLD_P95" in src_gen)
    expect("extra. resolve_role(row) in _generate_cards", "resolve_role(row)" in src_gen)
...
def build_parser():
    p = argparse.ArgumentParser(
        description="Explanation Card S3 Expansion v2 Font-Fix Full (300장) 카드 생성기 (가드 필수).")
